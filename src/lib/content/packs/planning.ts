import type { ContentPack } from '../types'

export const pack: ContentPack = {
  items: [
    {
      id: 'plan-rozhodnuti-mit-dite',
      kind: 'article',
      title: 'Rozhodnutí mít dítě: jak poznat, že je čas',
      excerpt:
        'Připravená na sto procent nebudete nikdy. Existují ale otázky, které je lepší si zodpovědět dřív než později.',
      body: `## Připravenost není pocit, je to rozhodnutí

Většina žen čeká na den, kdy si budou jisté. Ten den obvykle nepřijde. Přijde spíš okamžik, kdy si uvědomíte, že už nechcete čekat na jistotu. A že se rozhodujete i tím, že se nerozhodujete.

Tenhle text vám nemá říct, jestli chcete dítě. Má vám dát otázky, které vám pomůžou to zjistit.

## Čtyři otázky, které stojí za víc než všechny testy

**1. Chci dítě, nebo chci mít splněno?**
Rozdíl je zásadní. Tlak okolí, rodinné večeře a fotky kamarádek na sociálních sítích umí vytvořit pocit naléhavosti, který s vaším skutečným přáním nesouvisí. Zkuste si větu „chci dítě" říct nahlas, když jste sama. Znějí vám ta slova jako vaše?

**2. Co konkrétně mi na tom nahání strach?**
Kariéra, peníze, vztah, porod, ztráta svobody, vlastní matka. Strach pojmenovaný nahlas se scvrkne. Strach nepojmenovaný roste.

**3. Jak by vypadal náš život za dva roky?**
Ne ten ideální. Ten reálný. S vaším bytem, vaší prací, vaším partnerem nebo bez něj, s vaší podporou v rodině nebo bez ní.

**4. Co udělám, když to nepůjde hned?**
Tohle je otázka, kterou si skoro nikdo neklade předem. A přitom je to ta, která pak nejvíc bolí. Zamyslet se nad ní teď neznamená přivolávat problémy. Znamená to nebýt zaskočená.

## Věk je faktor, ne rozsudek

Plodnost se s věkem mění, to je biologie a nemá cenu to obcházet. Zásoba vajíček klesá a jejich kvalita se postupně zhoršuje, výrazněji zhruba od poloviny třicátých let. Zároveň to neznamená, že po třicátém pátém roce „už je pozdě". Spousta žen otěhotní bez potíží i výrazně později.

Co s tím prakticky:

- Pokud vám je nad 35 a víte, že dítě chcete, nemá smysl čekat další dva roky na „lepší chvíli".
- Pokud si chcete udělat obrázek o svém stavu, gynekolog vám může navrhnout základní vyšetření včetně ultrazvuku a hormonálních odběrů.
- Žádné číslo z laboratoře neřekne, jestli otěhotníte. Řekne jen, s jakým výchozím bodem pracujete.

## Peníze, práce a ostatní praktické věci

Tyhle věci lidé buď nadhodnocují, nebo je úplně vytěsňují. Realistický pohled vypadá takhle:

- **Rodičovská a mateřská** jsou v Česku poměrně štědré, ale první rok bývá finančně napjatý u většiny rodin. Podívejte se, jak by vypadal váš rozpočet s jedním hlavním příjmem.
- **Práce**: zjistěte, jak to u vás ve firmě chodí. Ne kvůli rozhodnutí, ale abyste věděla, s čím počítat.
- **Bydlení** nemusí být finální. Málokdo přivede dítě do vysněného domu.

## Když nejste ve shodě s partnerem

Nejčastější scénář není „on nechce". Bývá to „on ještě neví" nebo „on se bojí něčeho jiného než já". Nesnažte se ho přesvědčit argumenty o věku. Zeptejte se ho, co si představuje pod slovem otec a čeho se na tom bojí. Ten rozhovor obvykle otevře víc než tři měsíce naléhání.

## Co udělat tenhle týden

1. Napište si odpovědi na čtyři otázky výš. Rukou, ne do telefonu.
2. Objednejte se na preventivní gynekologickou prohlídku, pokud jste na ní přes rok nebyla.
3. Zjistěte, kdy jste naposledy měla kompletní krevní obraz a jaké máte očkování proti zarděnkám a planým neštovicím.
4. Řekněte partnerovi jednu větu o tom, co skutečně cítíte. Ne plán, jen pocit.

> Tento text má informativní charakter a nenahrazuje konzultaci s vaším lékařem. O svém zdravotním stavu a plánech se vždy poraďte s gynekologem, který vás zná.`,
      minutes: 7,
      phases: ['thinking'],
      topics: ['psychika', 'vztah', 'partner'],
      level: 'essential',
      hero: 'champagne',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['Česká gynekologická a porodnická společnost ČLS JEP', 'ESHRE: doporučené postupy'],
      publishedOn: '2025-09-04',
      boost: 0.35,
    },
    {
      id: 'plan-rozhovor-s-partnerem',
      kind: 'article',
      title: 'Jak o dítěti mluvit s partnerem, aby to nebyl souboj',
      excerpt:
        'Většina párů se nehádá o tom, jestli dítě chtějí. Hádají se o tempu, o strachu a o tom, kdo to má na starosti.',
      body: `## Proč tyhle rozhovory tak často skončí špatně

Když žena otevře téma dítěte, obvykle už ho v hlavě řeší několik měsíců. Partner ho v tu chvíli slyší poprvé. Vy jste v desáté minutě přemýšlení, on v první vteřině. To samo o sobě vytváří nedorozumění, které nemá nic společného s tím, jestli vás má rád.

Druhá věc: muži často reagují na téma dítěte přes odpovědnost a zajištění. „Ještě nemáme dost peněz" nebo „počkejme, až dodělám tu práci" bývá v překladu „bojím se, že to neutáhnu". Zní to jako odmítnutí. Většinou to není odmítnutí.

## Nastavte rozhovor tak, aby měl šanci

- **Ne po půlnoci a ne v autě cestou na návštěvu.** Vyberte si klidnou dobu, kdy nikam nespěcháte.
- **Řekněte předem, o čem to bude.** „Chci si s tebou o něčem promluvit, potřebuju na to hodinu" je férovější než přepadovka.
- **Začněte pocitem, ne plánem.** „Poslední dobou o tom hodně přemýšlím a je mi z toho úzko" otevírá dveře. „Musíme se domluvit, kdy vysadím antikoncepci" je zavírá.
- **Nedávejte ultimáta v prvním kole.** Ultimátum je poslední nástroj, ne první.

## Tři otázky, které posunou víc než hodina argumentů

1. **Co si představíš, když se řekne, že bys byl táta?** Odpověď vám řekne, jestli má vůbec nějakou představu. Spousta mužů ji nemá a je to normální.
2. **Čeho se na tom nejvíc bojíš?** Nechte ho domluvit, i když bude odpověď znít malicherně.
3. **Co by se muselo stát, abys byl klidnější?** Tady se často objeví něco velmi konkrétního a řešitelného.

## Když on chce počkat a vy ne

Tady nepomůže mlžení. Pomůže konkrétnost:

- **Pojmenujte časový rámec.** Ne „někdy", ale „chci to zkusit do konce příštího roku". Bez rámce se čekání táhne donekonečna.
- **Vysvětlete biologii bez vydírání.** Plodnost se s věkem mění a čas není neutrální. To je fakt, ne nátlak.
- **Navrhněte první krok, který ho nezavazuje.** Třeba společnou návštěvu gynekologa nebo předkoncepční vyšetření. Konkrétní malý krok bývá stravitelnější než velké rozhodnutí.
- **Zvažte pár sezení u párového terapeuta**, pokud se rozhovor opakovaně točí dokola. Není to selhání, je to zkratka.

## Rozdělte si role dřív, než začnete

Snažení o dítě se v naprosté většině párů promění v ženský projekt. Ona sleduje cyklus, ona objednává vyšetření, ona čte, ona pláče. Tomu se dá předejít, když si role rozdělíte hned na začátku:

- Kdo objednává termíny a hlídá kalendář.
- Kdo se stará o vlastní přípravu. Muž má svou část, spermie se obnovují zhruba tři měsíce a jeho životospráva má na výsledek prokazatelný vliv.
- Jak spolu budeme mluvit o výsledcích testů. Někdo chce vědět všechno hned, někdo až večer.
- Co uděláme, když přijde menstruace. Tenhle den se bude opakovat a je dobré mít předem domluveno, jestli chcete být sama, nebo v objetí.

## Signál, že je čas na odbornou pomoc ve vztahu

- Téma dítěte se stalo jediným, o čem spolu mluvíte.
- Jeden z vás se sexu vyhýbá.
- Objevují se výčitky typu „kvůli tobě" nebo „nebýt tebe".

Párová terapie v téhle fázi není luxus. Je to údržba něčeho, co budete potřebovat celou cestu.

> Tento text nenahrazuje odbornou psychologickou ani lékařskou péči. Pokud se vztah dlouhodobě zhoršuje, vyhledejte párového terapeuta.`,
      minutes: 8,
      phases: ['thinking', 'trying_naturally'],
      topics: ['vztah', 'partner', 'psychika'],
      level: 'deep',
      hero: 'taupe',
      author: 'Gabi',
      publishedOn: '2025-09-18',
    },
    {
      id: 'plan-tri-mesice-predem',
      kind: 'article',
      title: 'Tři měsíce předem: proč právě teď záleží na tom, co děláte',
      excerpt:
        'Vajíčko, které možná bude to vaše, dozrává zhruba devadesát dní. Právě proto má příprava těla smysl začít dnes.',
      body: `## Devadesát dní, o kterých se moc nemluví

Když se řekne příprava na těhotenství, většina lidí si představí vysazení antikoncepce a koupi folátu. Ve skutečnosti je nejzajímavější číslo tři měsíce.

Vajíčko, které se uvolní při ovulaci, neprocitne ze dne na den. Folikul, ve kterém dozrává, prochází několikaměsíčním vývojem. Poslední fáze zrání trvá zhruba osmdesát až devadesát dní. Podobně je to u muže: tvorba spermií trvá přibližně sedmdesát až devadesát dní. Znamená to, že **spermie, které se dnes podílejí na oplození, vznikaly zhruba před třemi měsíci.**

Praktický důsledek je hezký: to, co změníte dnes, se projeví na kvalitě buněk zhruba za čtvrt roku. A protože nikdo neví, který měsíc bude ten úspěšný, je každý den přípravy vkladem.

## Co má v těch třech měsících skutečně smysl

### 1. Doplňky, které mají oporu v doporučeních
Folát je jediný doplněk, u kterého panuje široká shoda, že by ho měla užívat každá žena plánující těhotenství, a to ideálně několik měsíců před početím. Dále se běžně řeší vitamin D a jód. Konkrétní dávky a kombinace ale patří do rukou lékaře. Liší se podle vaší anamnézy, hmotnosti, výsledků odběrů i toho, jaké další léky užíváte.

### 2. Základní vyšetření
Domluvte si s gynekologem předkoncepční kontrolu. Obvykle se řeší:

- gynekologická prohlídka a ultrazvuk,
- cytologie, pokud není aktuální,
- krevní obraz, železo, štítná žláza,
- imunita proti zarděnkám a planým neštovicím,
- kontrola chronických nemocí a užívaných léků.

Poslední bod je důležitější, než se zdá. Některé běžně užívané léky se v těhotenství nedoporučují a je potřeba je včas nahradit, ale nikdy je nevysazujte sama, vždy po domluvě s lékařem, který vám je předepsal.

### 3. Návyky, které mění výsledek
- **Alkohol**: při snaze o dítě je nejbezpečnější varianta žádný.
- **Kouření**: u obou partnerů zhoršuje plodnost. Tři měsíce je zároveň doba, za kterou se organismus po odvykání znatelně zotaví.
- **Kofein**: odborná doporučení se shodují spíš na omezení než na úplném vysazení. Konkrétní hranici proberte s lékařem.
- **Spánek**: pravidelný režim ovlivňuje hormonální rytmus víc, než většina žen čeká.
- **Hmotnost**: výrazný podváha i nadváha mohou narušovat ovulaci. Cílem nejsou drastické diety, ale postupná stabilizace.

### 4. Zubař
Zní to nesouvisle, ale zánět v dutině ústní se v těhotenství hůř řeší a zubní zákroky se pak často odkládají. Preventivní prohlídka teď vám ušetří starosti později.

## Čeho se naopak vzdát nemusíte

- **Kávy úplně.** Viz výše.
- **Cvičení.** Přiměřený pohyb plodnosti pomáhá. Škodí až extrémní zátěž kombinovaná s nízkou hmotností.
- **Sexu bez plánování.** Ve fázi přípravy ještě nemusíte nic měřit. Užijte si poslední měsíce, kdy to není práce.
- **Barvení vlasů, plavání v bazénu, letecké dovolené.** Nic z toho není překážkou početí.

## Jak si to rozvrhnout

**První měsíc:** doplňky, objednávka k lékaři, zubař, konec alkoholu.
**Druhý měsíc:** spánkový režim, pohyb, řešení nikotinu, kontrola léků.
**Třetí měsíc:** začněte poznávat svůj cyklus. Délka, charakter krvácení, orientační doba ovulace. Nemusíte nic zapisovat do tabulek, stačí vědomé pozorování.

## Kdy nečekat tři měsíce

Pokud vám je nad pětatřicet, máte nepravidelný cyklus, silné bolestivé menstruace, známou endometriózu, PCOS, onemocnění štítné žlázy nebo za sebou opakované ztráty těhotenství, objednejte se k lékaři rovnou a přípravu řešte souběžně.

> Tento článek má informativní charakter a nenahrazuje vyšetření ani péči vašeho lékaře. Doplňky stravy ani léky nezačínejte a nevysazujte bez konzultace.`,
      minutes: 9,
      phases: ['preparing_body', 'thinking'],
      dayRange: [0, 20],
      topics: ['strava', 'sebepece', 'hormony'],
      level: 'essential',
      hero: 'sage',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: [
        'ESHRE: doporučené postupy',
        'Česká gynekologická a porodnická společnost ČLS JEP',
        'Světová zdravotnická organizace. Doporučení pro předkoncepční péči',
      ],
      publishedOn: '2025-09-10',
      boost: 0.45,
    },
    {
      id: 'plan-folat-jod-vitamin-d',
      kind: 'article',
      title: 'Folát, vitamin D a jód: co má smysl doplňovat před početím',
      excerpt:
        'V lékárně na vás čeká třicet krabiček. Ve skutečnosti se odborná doporučení točí kolem tří věcí.',
      body: `## Proč vůbec něco doplňovat

Zdravá strava je základ, ale u některých látek je potřeba v období kolem početí vyšší, než se dá běžným jídelníčkem spolehlivě pokrýt. Zároveň platí, že víc neznamená lépe. Některé vitaminy se ve vysokých dávkách kumulují a mohou škodit. Proto je tenhle článek o tom, čemu rozumět, ne o tom, co si koupit.

**Konkrétní dávky a přípravky patří do rukou vašeho lékaře.** Liší se podle vaší anamnézy, hmotnosti, výsledků odběrů a dalších léků, které užíváte.

## Folát (kyselina listová)

Nejlépe doložený doplněk v celé předkoncepční péči. Podílí se na uzávěru neurální trubice plodu, což je děj, který proběhne velmi brzy. Často dřív, než žena vůbec ví, že je těhotná. Právě proto se doporučuje začít s doplňováním **před** početím, ne až po pozitivním testu.

Co je dobré vědět:

- Užívání se obvykle doporučuje zahájit několik týdnů až měsíců před plánovaným početím a pokračovat v prvním trimestru.
- Existuje forma kyseliny listové a forma metylovaného folátu. Rozdíl řeší některé ženy kvůli genetické variantě v metabolismu folátů. Jestli má u vás smysl to zjišťovat, posoudí lékař.
- **Vyšší dávku** lékař zvažuje například u žen s vrozenou vadou neurální trubice v rodinné anamnéze, s epilepsií, cukrovkou nebo obezitou. Nikdy si ji nenavyšujte sama.

## Vitamin D

V našich zeměpisných šířkách má nedostatek vitaminu D velká část populace, zvlášť v zimních měsících. Vitamin D souvisí s imunitou, kostním metabolismem a některými reprodukčními pochody.

- Hladinu lze změřit z krve. Jde o parametr, který se běžně vyšetřuje, a dává smysl znát ho ještě před těhotenstvím.
- **Dávkování se odvíjí od naměřené hladiny.** Vitamin D je rozpustný v tucích a předávkovat se dá, proto se nedoplňuje odhadem.

## Jód

Potřeba jódu v těhotenství roste, protože se podílí na tvorbě hormonů štítné žlázy, které jsou zásadní pro vývoj mozku plodu. V Česku je jodizovaná sůl, ale mnoho žen sůl vědomě omezuje.

- Většina kvalitních prenatálních přípravků jód obsahuje. **Zkontrolujte složení** toho svého.
- Pozor na doplňky z mořských řas. Obsah jódu v nich bývá neznámý a někdy extrémně vysoký.
- Pokud se léčíte se štítnou žlázou, doplnění jódu vždy konzultujte s endokrinologem.

## Co ještě lékař někdy řeší

- **Železo**: při nízkém feritinu, který je u menstruujících žen častý.
- **Omega-3 mastné kyseliny**: zejména při nízkém příjmu ryb.
- **Vitamin B12**: u vegetariánek a veganek prakticky vždy.

## Čemu se vyhnout

- **Vitaminu A ve vysokých dávkách** (a přípravkům s retinolem). Ve vyšších dávkách je v těhotenství rizikový.
- **Bylinným směsím „na plodnost"** s nejasným složením. Nemají doložený efekt a mohou interagovat s léky.
- **Kombinaci pěti různých přípravků najednou.** Snadno se tak dostanete k několikanásobku doporučeného množství stejné látky.

## Jak si to zjednodušit

1. Vezměte si na kontrolu k lékaři krabičky všeho, co užíváte, včetně bylinek a čajů.
2. Nechte si zkontrolovat štítnou žlázu, krevní obraz, feritin a vitamin D.
3. Vyberte jeden komplexní prenatální přípravek podle doporučení lékaře místo pěti samostatných.
4. Nastavte si připomínku na stejnou denní dobu. Pravidelnost je důležitější než značka.

> Tento text je informativní a nenahrazuje lékařskou péči. Žádný doplněk stravy nezačínejte užívat ve vyšších dávkách bez konzultace s lékařem.`,
      minutes: 8,
      phases: ['preparing_body', 'thinking', 'trying_naturally'],
      dayRange: [0, 30],
      topics: ['strava', 'leky', 'hormony'],
      level: 'essential',
      hero: 'linen',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: [
        'Světová zdravotnická organizace. Doporučení pro předkoncepční péči',
        'Česká gynekologická a porodnická společnost ČLS JEP',
        'Státní zdravotní ústav. Výživová doporučení',
      ],
      publishedOn: '2025-09-25',
      boost: 0.4,
    },
    {
      id: 'plan-alkohol-koureni-kofein',
      kind: 'article',
      title: 'Alkohol, cigarety a káva: co opravdu musí pryč',
      excerpt:
        'Jedno z toho je jednoznačné, druhé je jednoznačné taky a to třetí si nejspíš můžete nechat.',
      body: `## Nejdřív to podstatné

Tenhle článek není o vině. Když teď čtete, že alkohol nebo cigarety škodí, a zároveň víte, že jste minulý měsíc pila víno, nic jste nezničila. Buňky se obnovují, tělo se zotavuje a **od dnešního dne máte tři měsíce na to, aby se změna projevila.**

## Alkohol

Nejbezpečnější množství alkoholu při snaze o dítě i v těhotenství je **žádné**. Neexistuje spolehlivě určená bezpečná hranice a alkohol prochází placentou.

Praktický problém je, že mezi početím a pozitivním testem uplynou dva až tři týdny, kdy o těhotenství ještě nevíte. Právě proto se doporučuje vysadit alkohol už ve fázi snažení, ne až po testu.

Co s tím prakticky:

- Připravte si jednu větu pro společnost. „Dnes nepiju, mám ráno práci" stačí a nikdo se dál neptá.
- Mějte doma vlastní nealko variantu, kterou máte skutečně ráda, ne jen minerálku z nutnosti.
- Pokud je pro vás vysazení opravdu těžké nebo pijete denně, řekněte to lékaři. Není to ostuda, je to informace, kterou potřebuje.

## Kouření

Kouření zhoršuje plodnost u obou partnerů. U žen se popisuje rychlejší úbytek vaječníkové rezervy, u mužů horší parametry spermiogramu. Souvisí také s vyšším rizikem mimoděložního těhotenství a potratu.

- Platí to i pro **pasivní kouření**: pokud kouří partner v bytě, týká se to i vás.
- **Zahřívaný tabák a elektronické cigarety nejsou bezpečná varianta.** Data jsou zatím omezená a nikotin sám o sobě není neutrální.
- Odvykání je v tomhle období těžší kvůli stresu. V Česku existují centra pro závislé na tabáku a linka pro odvykání. A hlavně: **náhradní nikotinovou terapii v době snažení konzultujte s lékařem**, protože ne každá forma je vhodná.

Dobrá zpráva: zotavení organismu po odvykání je poměrně rychlé a parametry spermiogramu se u mužů obvykle zlepšují během několika měsíců.

## Kofein

Tady se nejčastěji šíří panika, která nemá oporu. Odborná doporučení se shodují spíš na **omezení než na úplném vysazení** kofeinu při snaze o dítě a v těhotenství. Přesnou hranici, která je vhodná pro vás, vám řekne lékař. A je rozumné ji znát dřív, než si sama zakážete i ranní kávu.

Na co se ale často zapomíná:

- Kofein není jen v kávě. Je v černém i zeleném čaji, v kolových nápojích, v energetických nápojích, v hořké čokoládě a v některých volně prodejných lécích proti bolesti hlavy.
- **Energetické nápoje** jsou v tomhle období tou nejméně vhodnou variantou. Kombinují vysoký obsah kofeinu s dalšími stimulanty.
- Pokud kávu používáte hlavně na to, abyste přežila den, řešte spíš spánek než kávu.

## Co se řeší zbytečně

- **Barvení vlasů.** Není důvod se ho vzdávat.
- **Sauna a horká koupel u žen.** Nejsou překážkou početí. U mužů má naopak smysl přehřívání varlat omezit.
- **Mobilní telefon v kapse u ženy.** Nemá to prokázaný vliv na plodnost.
- **Jedno sushi.** Riziko je v syrových a nedostatečně tepelně upravených potravinách obecně, ne v jednom druhu jídla. A to se týká hlavně těhotenství, kde jde o listerie a toxoplazmózu.

## Plán na tenhle týden

1. Vypište si, kolik alkoholu jste vypila za poslední měsíc. Bez hodnocení, jen fakta.
2. Domluvte se s partnerem, že přestáváte oba. Sama to jde hůř.
3. Spočítejte si zdroje kofeinu za den. Obvykle jich je víc, než čekáte.
4. Pokud kouříte, stanovte konkrétní datum posledního dne. Ne „časem".

> Tento text nenahrazuje lékařskou péči. Pokud řešíte závislost na alkoholu či nikotinu, obraťte se na svého lékaře nebo na specializované centrum. V období plánování těhotenství to má o to větší smysl.`,
      minutes: 7,
      phases: ['preparing_body', 'trying_naturally', 'thinking'],
      dayRange: [0, 45],
      topics: ['strava', 'sebepece', 'partner'],
      level: 'essential',
      hero: 'sand',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: [
        'Světová zdravotnická organizace. Doporučení pro předkoncepční péči',
        'Česká gynekologická a porodnická společnost ČLS JEP',
      ],
      publishedOn: '2025-10-02',
    },
    {
      id: 'plan-vaha-pohyb-plodnost',
      kind: 'article',
      title: 'Váha a pohyb: kde je hranice mezi pomocí a škodou',
      excerpt:
        'Tuková tkáň je hormonálně aktivní orgán, proto ovlivňuje ovulaci na obou koncích váhy, nahoře i dole.',
      body: `## Proč váha vůbec souvisí s ovulací

Tuková tkáň není jen zásoba energie. Je hormonálně aktivní. Podílí se na přeměně a hladině pohlavních hormonů a komunikuje s centrem v mozku, které řídí menstruační cyklus. Proto může **výrazná nadváha i výrazná podváha** narušit ovulaci, aniž by byl kdekoliv jinde nějaký problém.

Zdůrazněme dvě slova: výrazná a může. Neplatí, že každá žena s vyšší váhou má potíže s otěhotněním. Neplatí ani, že hubená žena je automaticky plodnější.

## Co znamená „výrazná"

Lékaři obvykle pracují s indexem tělesné hmotnosti (BMI), kde se pásmo běžně označované jako normální pohybuje zhruba mezi hodnotami 18,5 a 25. Je to hrubé měřítko, které nerozlišuje svalovou hmotu a nezohledňuje rozložení tuku, proto se na něm samotném nedá stavět.

Co je při snaze o dítě relevantnější než jedno číslo:

- **Pravidelnost cyklu.** Pravidelná menstruace je nepřímá známka toho, že ovulace probíhá.
- **Trend.** Rychlý úbytek nebo přírůstek hmotnosti v posledním roce ovlivňuje cyklus víc než stabilní vyšší váha.
- **Kontext PCOS a inzulinové rezistence.** U syndromu polycystických ovarií má i mírná redukce hmotnosti často znatelný vliv na obnovení ovulace. Konkrétní postup patří k vašemu gynekologovi nebo endokrinologovi.

## Redukce, která pomůže, a redukce, která uškodí

**Pomáhá:** postupná změna jídelníčku, pravidelnost jídel, dostatek bílkovin a vlákniny, mírný energetický deficit, který udržíte měsíce.

**Škodí:** drastické diety, hladovění, vyřazení celých skupin potravin bez důvodu, redukce o mnoho kilogramů za pár týdnů. Prudký úbytek hmotnosti je pro tělo stresová situace a cyklus na něj reaguje jako první.

Pokud plánujete výraznější redukci, proberte časování s lékařem. Někdy je rozumnější zhubnout **před** začátkem snažení než uprostřed něj.

## Pohyb: kolik je akorát

Přiměřený pravidelný pohyb zlepšuje citlivost na inzulin, náladu, spánek i schopnost zvládat stres. To všechno hraje ve váš prospěch.

Rozumný rámec vypadá takhle:

- **Většina dnů v týdnu nějaký pohyb.** Rychlá chůze se počítá.
- **Silový trénink dvakrát týdně.** Pomáhá metabolismu i tomu, jak se budete cítit v těhotenství.
- **Intenzitu volte tak, abyste se den poté cítila líp, ne hůř.**

### Kdy je pohybu moc

Extrémní vytrvalostní zátěž v kombinaci s nízkým příjmem energie může vést k tomu, že cyklus zeslábne nebo úplně vymizí. Odborně se tomu říká relativní energetický deficit ve sportu. Varovné signály:

- menstruace se zkrátila, zeslábla nebo vynechává,
- únava, která nemizí po odpočinku,
- opakovaná zranění, časté nemoci,
- neustálý hlad nebo naopak jeho úplná ztráta.

Pokud se v tomhle poznáváte, není řešením trénovat ještě víc. Je řešením přidat jídlo a probrat to s lékařem.

## Co dělat, když se s váhou perete dlouhodobě

Vztah k jídlu bývá u žen v přípravě na těhotenství citlivé místo. Pokud za sebou máte poruchu příjmu potravy nebo roky diet, řekněte to gynekologovi. Není to detail. Je to informace, která mění doporučení.

## Malý plán, který jde udržet

1. Tři procházky týdně po třiceti minutách, pevně v kalendáři.
2. Dvakrát týdně silový trénink, klidně doma a krátký.
3. Bílkovina v každém hlavním jídle.
4. Žádné vážení každý den. Jednou týdně stačí, a i to je nepovinné.

> Tento článek nenahrazuje péči lékaře ani nutričního terapeuta. Výraznější změny hmotnosti v období plánování těhotenství vždy konzultujte se svým gynekologem.`,
      minutes: 8,
      phases: ['preparing_body', 'trying_naturally'],
      dayRange: [10, 60],
      topics: ['strava', 'pohyb', 'sebepece'],
      level: 'deep',
      hero: 'sage',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['ESHRE (doporučené postupy', 'Státní zdravotní ústav) výživová doporučení'],
      publishedOn: '2025-10-09',
    },
    {
      id: 'plan-spanek-a-plodnost',
      kind: 'article',
      title: 'Spánek: nejpodceňovanější část přípravy těla',
      excerpt:
        'Hormonální osa, která řídí váš cyklus, se řídí světlem a tmou. Nespavost proto není jen otázka únavy.',
      body: `## Co má spánek společného s cyklem

Menstruační cyklus řídí osa mezi hypotalamem, hypofýzou a vaječníky. Tahle osa je citlivá na denní rytmus, světlo a stresové hormony. Když se váš režim rozhodí (směnný provoz, chronické ponocování, buzení v noci) mění se i vylučování hormonů, které cyklus řídí.

Zároveň platí, že spánek ovlivňuje inzulinovou citlivost, chuť k jídlu a schopnost snášet stres. To všechno se v období snažení počítá.

## Kolik a hlavně jak

Většina dospělých potřebuje sedm až devět hodin. Důležitější než celkový počet hodin je ale **pravidelnost**. Tělo si hormonální rytmus staví na tom, že chodíte spát a vstáváte přibližně ve stejnou dobu. I o víkendu.

Praktický rámec:

- **Stejný čas vstávání** sedm dní v týdnu. Tohle je jediná věc, kterou máte pod kontrolou, i když usínání ne.
- **Ranní světlo.** Deset minut venku dopoledne ukotví rytmus lépe než jakýkoli doplněk.
- **Tma večer.** Tlumené osvětlení hodinu před spaním, telefon co nejdál od postele.
- **Chladná ložnice.** Teplota kolem osmnácti stupňů je pro většinu lidí příjemnější, než čekají.

## Když nemůžete spát kvůli hlavě

Tohle je v období snažení nejčastější důvod. Ležíte, počítáte dny do testu a přehráváte si scénáře.

Co funguje líp než snaha usnout:

- **Vypsat to.** Deset minut před spaním si na papír vypište všechno, co máte v hlavě. Není to deník, je to vyskladnění.
- **Odložený čas na starosti.** Vyhraďte si patnáct minut odpoledne, kdy se smíte trápit naplno. Večerní hlava pak má kam odkázat.
- **Vstát po dvaceti minutách.** Když neusínáte, nezůstávejte v posteli. Postel se nesmí stát místem, kde se bojujete.
- **Dechové cvičení s delším výdechem.** Nádech na čtyři, výdech na šest, pár minut. Fyziologicky tlumí aktivaci.

## Melatonin a další doplňky

Melatonin je v Česku dostupný ve volném prodeji, ale **jeho užívání v období snažení o dítě konzultujte s lékařem**: týká se to i bylinných přípravků na spaní. To samé platí pro léky na spaní na předpis, které v těhotenství vhodné nemusí být.

## Směnný provoz a noční práce

Pokud pracujete na směny, nemáte možnost si režim jen tak srovnat. Realistické kroky:

- Chraňte si spánek po noční. Tma, špunty, telefon na tichý.
- Po noční se snažte dostat na světlo až v době, kdy plánujete zůstat vzhůru.
- Řekněte to gynekologovi. Je to relevantní informace pro posouzení nepravidelného cyklu.
- Pokud vám to zaměstnavatel umožní, zvažte v období snažení dočasnou úpravu rozpisu.

## Kdy nejde o „špatný spánek", ale o problém

Poraďte se s lékařem, pokud:

- chrápete a partner popisuje, že se vám během spánku zastavuje dech,
- usínáte přes den i po dostatku spánku,
- nespavost trvá déle než měsíc a ovlivňuje váš běžný den,
- budíte se v noci zpocená a s bušením srdce,
- máte pocit trvalé úzkosti, který nepolevuje.

## Tři věci na dnešní večer

1. Nastavte si alarm ne na ráno, ale na večer. Připomínku, že za hodinu jdete spát.
2. Dejte nabíječku telefonu mimo ložnici.
3. Napište na papír tři věci, které vás dnes zaměstnávaly. Papír nechte v obýváku.

> Tento text nenahrazuje lékařskou péči. Přípravky na spaní ani melatonin neužívejte v období plánování těhotenství bez konzultace s lékařem.`,
      minutes: 7,
      phases: ['preparing_body', 'trying_naturally'],
      dayRange: [5, 60],
      topics: ['spanek', 'sebepece', 'hormony', 'psychika'],
      level: 'deep',
      hero: 'dusk',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['ESHRE: doporučené postupy'],
      publishedOn: '2025-10-16',
    },
    {
      id: 'plan-muzska-plodnost-90-dni',
      kind: 'article',
      title: 'Mužská plodnost: devadesát dní, které má v rukou on',
      excerpt:
        'Přibližně polovina případů neplodnosti má podíl na straně muže. A spermie se obnovují každé tři měsíce.',
      body: `## Proč tenhle text čtete vy a ne on

Statisticky ho čtete vy. Předkoncepční přípravu v drtivé většině párů řeší žena, přitom **na straně muže se podílí faktor zhruba v polovině případů neplodnosti**: buď samostatně, nebo v kombinaci s ženským faktorem.

Dobrá zpráva pro něj: mužská příprava je jednodušší než ta ženská a má rychlejší návratnost. Tvorba spermií trvá zhruba sedmdesát až devadesát dní, takže **změny životosprávy se projeví na spermiogramu přibližně za tři měsíce.**

Pošlete mu tenhle článek. Nebo mu ho přečtěte. Obojí je lepší než to řešit sama.

## Co spermiím prokazatelně škodí

### Teplo
Varlata jsou mimo tělo z důvodu. Potřebují nižší teplotu než zbytek organismu. Co má smysl omezit:

- pravidelnou saunu a horké koupele,
- vyhřívané sedačky v autě při dlouhých jízdách,
- notebook na klíně,
- příliš těsné spodní prádlo a kalhoty.

Nejde o to, že jedna sauna zničí půl roku snažení. Jde o dlouhodobý a pravidelný návyk.

### Kouření a alkohol
Obojí zhoršuje počet, pohyblivost i tvar spermií. U alkoholu platí, že škodí především pravidelná a vyšší konzumace. Kouření včetně marihuany má na spermiogram doložený negativní vliv.

### Anabolika a doplňky s testosteronem
Tohle je bod, který spousta mužů zamlčí. **Užívání testosteronu nebo anabolických steroidů výrazně tlumí vlastní tvorbu spermií** a účinek může přetrvávat dlouhé měsíce po vysazení. Pokud je partner užívá nebo užíval, musí to říct lékaři. Není to detail, je to často klíč k celé diagnóze.

### Nadváha, sedavé zaměstnání, chronický stres
Všechno tři jsou faktory, které se dají ovlivnit a jejichž vliv je popsaný.

### Některé léky
Sem patří například část léků na vysoký tlak, na vypadávání vlasů, na psychiatrickou medikaci a některé léky proti bolesti. **Žádný lék nevysazujte sám**: jen o nich řekněte lékaři.

## Co spermiím pomáhá

- **Pravidelný pohyb** v rozumné míře.
- **Strava bohatá na zeleninu, ryby, ořechy a olivový olej.** Středomořský typ jídelníčku má v téhle oblasti nejlepší podklad.
- **Dostatek spánku.**
- **Pravidelný sexuální život.** Dlouhá abstinence kvalitu spermatu nezlepšuje, spíš naopak. Optimální frekvenci pro váš případ řekne lékař podle výsledku spermiogramu.

Doplňky stravy „na plodnost pro muže" bývají směsi antioxidantů. Jejich efekt není jednoznačně prokázaný a **konkrétní přípravek i dávkování je na lékaři.**

## Kdy má jít na spermiogram

Nemá smysl čekat rok. Spermiogram je levné, neinvazivní a rychlé vyšetření a dá se udělat kdykoliv.

Konkrétní důvody jít dřív:

- v anamnéze operace varlat, nesestouplé varle, torze varlete,
- proděláná parotitida (příušnice) v dospělosti,
- křečové žíly na varleti (varikokéla),
- onkologická léčba v minulosti,
- opakované záněty močových cest nebo pohlavně přenosné infekce,
- potíže s erekcí nebo ejakulací.

## Jak se na spermiogram připravit

Konkrétní pokyny vám dá laboratoř, obecně se ale řeší doba pohlavní abstinence před odběrem, čerstvost vzorku a to, aby muž nebyl akutně nemocný nebo po horečce. **Horečnaté onemocnění může výsledek zhoršit na několik týdnů až měsíců**, proto se vyšetření po nemoci obvykle odkládá.

Jedno špatné vyšetření navíc neznamená diagnózu. Parametry kolísají a při odchylce se **vyšetření obvykle opakuje s odstupem**.

## Jak s ním o tom mluvit

Pro spoustu mužů je téma plodnosti přímo spojené s mužností. To není hloupost, to je kulturní nastavení, se kterým vyrostli.

Co pomáhá:

- Mluvit o tom jako o **společné diagnostice**, ne o jeho testu.
- Nabídnout mu konkrétní krok, ne obecné „měl bys s tím něco dělat".
- Nepoužívat výsledek jako argument v hádce. Nikdy.

> Tento text je informativní a nenahrazuje lékařskou péči. Výsledky vyšetření vždy interpretuje lékař. Nejlépe androlog nebo urolog se zaměřením na reprodukci.`,
      minutes: 9,
      phases: ['preparing_body', 'trying_naturally', 'thinking'],
      dayRange: [0, 60],
      topics: ['partner', 'vztah', 'strava'],
      level: 'essential',
      hero: 'taupe',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: [
        'ESHRE: doporučené postupy',
        'Světová zdravotnická organizace. Laboratorní manuál pro vyšetření ejakulátu',
        'Česká urologická společnost ČLS JEP',
      ],
      publishedOn: '2025-10-23',
      boost: 0.35,
    },
    {
      id: 'plan-spermiogram-vysledky',
      kind: 'article',
      title: 'Spermiogram: co ta čísla vlastně znamenají',
      excerpt:
        'Objem, koncentrace, motilita, morfologie. Čtyři pojmy, které rozhodují o tom, co bude dál.',
      body: `## Než se podíváte na papír

Spermiogram je základní vyšetření mužské plodnosti. Hodnotí se u něj množství a vlastnosti spermií ve vzorku ejakulátu. Laboratoře porovnávají výsledky s referenčními hodnotami, které vycházejí z mezinárodních doporučení.

Dvě věci, které je potřeba vědět dřív, než se leknete:

1. **Referenční hodnoty nejsou hranice mezi plodným a neplodným.** Vycházejí z toho, jaké hodnoty měli muži, jejichž partnerky otěhotněly během roku. Hodnota mírně pod hranicí neznamená, že početí není možné.
2. **Parametry kolísají.** Nemoc, horečka, stres, doba abstinence před odběrem i způsob odběru výsledek ovlivní. Proto se odchýlený nález obvykle **opakuje s několikatýdenním odstupem.**

## Co se hodnotí

### Objem
Množství ejakulátu. Velmi nízký objem může souviset s odběrem (část vzorku se nezachytila) nebo s překážkou ve vývodných cestách.

### Koncentrace a celkový počet
Kolik spermií je v mililitru a kolik celkem. Označení, se kterými se můžete setkat:

- **normozoospermie**: všechny sledované parametry v referenčním pásmu,
- **oligozoospermie**: snížený počet spermií,
- **azoospermie**: ve vzorku nebyly nalezeny žádné spermie,
- **kryptozoospermie**: spermie jsou nalezeny až po zpracování vzorku.

### Motilita (pohyblivost)
Podíl spermií, které se pohybují, a jak. Snížená pohyblivost se označuje jako **asthenozoospermie**.

### Morfologie (tvar)
Podíl spermií s normálním tvarem. Hodnotí se přísně a i u zdravých mužů bývá podíl normálních forem překvapivě nízký. Snížený podíl se označuje jako **teratozoospermie**.

Kombinace se pojmenovávají složeně, například **oligoasthenoteratozoospermie** (OAT). Zní to hrozivě, ale je to jen popis tří parametrů najednou.

### Další parametry
Vitalita, přítomnost leukocytů (mohou svědčit pro zánět), pH, viskozita. Někdy se doplňuje **test fragmentace DNA spermií**: to je samostatné vyšetření, které se neprovádí rutinně a jeho indikaci posuzuje lékař.

## Co z výsledku plyne pro další postup

Tohle je bod, kde je potřeba být opatrná: **z výsledku nelze vyvozovat léčebný postup bez lékaře.** Obecně ale platí, že nález ovlivňuje, jestli má smysl pokračovat v přirozeném snažení, jestli přichází v úvahu inseminace, nebo jestli se rovnou uvažuje o IVF s metodou ICSI, kdy se spermie vpravuje přímo do vajíčka.

Při azoospermii následuje další diagnostika. Hormonální vyšetření, genetické vyšetření a případně odběr tkáně z varlete. I v takové situaci existují cesty.

## Co může výsledek zlepšit

- odstranění tepelné zátěže,
- konec kouření a omezení alkoholu,
- léčba zjištěného zánětu nebo varikokély, pokud to lékař indikuje,
- úprava hmotnosti,
- vysazení anabolik. Vždy pod dohledem lékaře,
- čas: efekt změn se hodnotí nejdřív po zhruba třech měsících.

## Jak o výsledku mluvit doma

Nález mimo referenční pásmo bere spousta mužů jako verdikt o sobě samém. Pomáhá říct nahlas dvě věci: že je to **měřitelný a často ovlivnitelný parametr**, a že je to **společná záležitost**, ne jeho selhání.

Co nedělat: neposílejte mu výsledek v esemesce, neinterpretujte ho podle internetu a nesrovnávejte ho s cizími čísly z diskusí.

> Tento text má informativní charakter a nenahrazuje lékařské vyšetření. Výsledek spermiogramu vždy interpretuje lékař v kontextu celého páru.`,
      minutes: 8,
      phases: ['trying_naturally', 'preparing_body'],
      topics: ['partner', 'vysledky', 'klinika'],
      modifiers: ['male_factor'],
      level: 'deep',
      hero: 'sky',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: [
        'Světová zdravotnická organizace. Laboratorní manuál pro vyšetření ejakulátu',
        'ESHRE: doporučené postupy',
      ],
      publishedOn: '2025-11-06',
    },
    {
      id: 'plan-sledovani-cyklu',
      kind: 'article',
      title: 'Poznejte svůj cyklus: základ, na kterém stojí všechno ostatní',
      excerpt:
        'Než začnete měřit teplotu a kupovat testy, potřebujete vědět jednu věc. Jak dlouhý a jak pravidelný je váš cyklus.',
      body: `## Začněte tím nejjednodušším

Většina žen přeskočí základ a rovnou si koupí ovulační testy. Přitom nejcennější informaci získáte z obyčejného zápisu **prvního dne menstruace** po dobu tří měsíců.

První den cyklu = první den skutečného krvácení, ne špinění. Délka cyklu = počet dní od prvního dne jedné menstruace do dne před další.

Z těch tří čísel poznáte:

- jestli je váš cyklus pravidelný,
- jak dlouhý zhruba je,
- kam přibližně padá ovulace.

## Jak cyklus funguje

Cyklus má dvě fáze, oddělené ovulací.

**První fáze (folikulární)** začíná menstruací. Ve vaječníku dozrává folikul, roste hladina estrogenu a v děloze se buduje sliznice. **Tahle fáze je proměnlivá**: může trvat deset dní i tři týdny a právě ona rozhoduje o tom, jak dlouhý cyklus bude.

**Ovulace**: folikul praskne a uvolní vajíčko. To je schopné oplození zhruba dvanáct až dvacet čtyři hodin.

**Druhá fáze (luteální)** trvá u většiny žen poměrně stabilně kolem dvanácti až čtrnácti dní. Žluté tělísko tvoří progesteron, který připravuje sliznici na uhnízdění. Pokud k otěhotnění nedojde, hladina progesteronu klesne a přijde menstruace.

Praktický důsledek, který stojí za zapamatování: **ovulace je zhruba čtrnáct dní PŘED další menstruací, ne čtrnáct dní PO té minulé.** U cyklu délky 32 dní tedy ovulace nepadá na 14. den, ale spíš kolem 18. dne.

## Co si zapisovat

Nemusíte vést tabulky. Stačí:

1. **První den menstruace.**
2. **Délku a sílu krvácení.**
3. **Charakter hlenu** (podrobněji v samostatném článku).
4. **Bolesti**: kdy, kde, jak silné.
5. **Dny pohlavního styku.** Bez hodnocení, jen fakt.

Po třech měsících máte podklad, který lékaři řekne víc než hodina vyprávění.

## Aplikace: k čemu jsou a k čemu ne

Aplikace jsou skvělé na **zaznamenávání**. Jsou nespolehlivé v **předpovídání ovulace**, protože obvykle jen počítají průměr z minulých cyklů. Váš letošní březen ale o dubnu nic neví.

Berte předpověď v aplikaci jako orientaci, ne jako fakt. Skutečné potvrzení ovulace dá jen bazální teplota, ultrazvuk nebo hormonální odběr.

## Co je ještě v normě a co už ne

Za obvyklé rozmezí délky cyklu se považuje zhruba 21 až 35 dní. Odchylka o pár dní mezi cykly je běžná.

**Objednejte se ke gynekologovi, pokud:**

- je cyklus kratší než 21 dní nebo delší než 35 dní,
- délka cyklů se mezi sebou liší o víc než zhruba týden,
- menstruace vynechává úplně,
- krvácení je tak silné, že musíte měnit ochranu častěji než po dvou hodinách, nebo obsahuje velké koaguly,
- máte silné bolesti, které vás vyřazují z běžného dne, nebo bolesti mimo menstruaci,
- objevuje se špinění mezi cykly,
- bolí vás pohlavní styk.

Nepravidelnost není vaše vina ani znak slabosti. Je to informace. A často se za ní skrývá něco, co má řešení, například porucha štítné žlázy nebo PCOS.

## První tři kroky

1. Zapište si datum poslední menstruace ještě dnes.
2. Nastavte si připomínku, abyste zápis nezapomněla ani u příští.
3. Za tři měsíce se podívejte na čísla. A pokud sedí do kolonek výš, objednejte se k lékaři.

> Tento text má informativní charakter a nenahrazuje gynekologické vyšetření. Nepravidelný cyklus vždy proberte se svým lékařem.`,
      minutes: 8,
      phases: ['trying_naturally', 'preparing_body'],
      dayRange: [0, 60],
      topics: ['hormony', 'vysledky', 'sebepece'],
      level: 'essential',
      hero: 'blush',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['Česká gynekologická a porodnická společnost ČLS JEP', 'ESHRE: doporučené postupy'],
      publishedOn: '2025-11-13',
      boost: 0.45,
    },
    {
      id: 'plan-bazalni-teplota',
      kind: 'article',
      title: 'Bazální teplota: co vám řekne a co od ní nečekat',
      excerpt:
        'Teplota ovulaci potvrdí, ale nepředpoví ji. Pochopit tenhle rozdíl vám ušetří měsíce zbytečné frustrace.',
      body: `## Jak to funguje

Po ovulaci začne žluté tělísko tvořit progesteron. Progesteron mírně zvyšuje tělesnou teplotu. Obvykle zhruba o dvě až pět desetin stupně. Když si teplotu měříte každé ráno, uvidíte v grafu **dvoufázový průběh**: nižší hodnoty v první fázi cyklu, skok nahoru po ovulaci a udržení vyšší hladiny až do menstruace.

A tady je ta zásadní věta: **teplota stoupne až PO ovulaci.** Když ji vidíte, plodné okno se právě zavírá. Bazální teplota tedy slouží k **potvrzení**, že ovulace proběhla, a k poznání vzorce vašeho cyklu, ne k načasování styku v daném měsíci.

Po několika měsících ale získáte něco cenného: víte, kdy u vás ovulace obvykle nastává, a příště podle toho můžete plánovat.

## Jak měřit, aby to mělo cenu

- **Hned po probuzení**, ještě než vstanete, napijete se nebo se posadíte.
- **Ve stejnou dobu**, odchylka do půl hodiny je v pořádku.
- **Po alespoň třech hodinách souvislého spánku.**
- **Stejným teploměrem a stejnou cestou**: vaginálně, rektálně nebo v ústech. Metody se nemíchají. Digitální teploměr s dvěma desetinnými místy je praktičtější než klasický.
- **Zapisujte i okolnosti**: nemoc, alkohol, špatně prospaná noc, cesta přes časová pásma. Tyhle dny se pak v grafu dají vyloučit.

## Co v grafu hledat

1. **Dvoufázovost.** Jasně viditelný předěl znamená, že ovulace nejspíš proběhla.
2. **Délku druhé fáze.** Od skoku teploty do menstruace. Opakovaně velmi krátká druhá fáze je něco, co má smysl zmínit lékaři.
3. **Udržení vyšší teploty déle, než je obvyklé.** Pokud teplota zůstává nahoře výrazně přes očekávaný termín menstruace, může jít o těhotenství. Potvrdí to test, ne graf.

## Co graf NEUMÍ

- **Neřekne dopředu, kdy bude ovulace.**
- **Nepotvrdí těhotenství.** Žádný tvar křivky není diagnóza.
- **Nezobrazí kvalitu vajíčka ani sliznice.**
- **Nenahradí lékařské vyšetření.** Interpretace „krátká luteální fáze = nedostatek progesteronu" je zkratka, kterou si nelze udělat sama.

## Kdy měření naopak škodí

Buďme upřímné: bazální teplota nesedí každé. Pokud v sobě poznáváte tohle, zvažte, jestli měřit:

- vzbudíte se ve čtyři ráno a nemůžete usnout, protože musíte v šest měřit,
- graf kontrolujete desetkrát denně,
- po každém zaváhání teploty se propadnete do úzkosti,
- pracujete na směny a hodnoty jsou stejně nepoužitelné.

**Měření je nástroj, ne povinnost.** Pokud vám bere víc klidu, než dává informace, klidně ho na pár cyklů odložte, nebo rovnou vsaďte na ovulační testy a sledování hlenu, které jsou pro plánování praktičtější.

## Kombinace, která dává největší smysl

Nejvíc informací získáte, když spojíte tři věci:

- **cervikální hlen**: předpovídá blížící se plodné dny,
- **ovulační test (LH)**: zachytí vzestup hormonu před ovulací,
- **bazální teplota**: zpětně potvrdí, že ovulace proběhla.

## Co říct lékaři

Vezměte grafy z posledních tří cyklů na kontrolu. Pro gynekologa je to použitelná informace, hlavně tehdy, když je křivka opakovaně jednofázová, druhá fáze velmi krátká nebo cyklus výrazně kolísá.

> Tento článek je informativní a nenahrazuje lékařskou péči. Grafy bazální teploty neslouží k diagnostice. Vždy je posuzuje lékař v kontextu dalších vyšetření.`,
      minutes: 7,
      phases: ['trying_naturally'],
      dayRange: [0, 120],
      topics: ['hormony', 'vysledky', 'cekani'],
      level: 'deep',
      hero: 'dawn',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['ESHRE: doporučené postupy', 'Česká gynekologická a porodnická společnost ČLS JEP'],
      publishedOn: '2025-11-20',
    },
    {
      id: 'plan-ovulacni-testy-lh',
      kind: 'article',
      title: 'Ovulační testy: jak je používat, aby k něčemu byly',
      excerpt:
        'LH test zachytí hormonální signál zhruba den až den a půl před ovulací, pokud ho ovšem netestujete ve špatnou dobu.',
      body: `## Co test měří

Ovulační test detekuje v moči **luteinizační hormon (LH)**. Jeho hladina prudce vystoupá krátce před ovulací (takzvaný LH vzestup) a ovulace obvykle následuje zhruba za dvacet čtyři až třicet šest hodin.

Právě proto je LH test praktičtější než bazální teplota: **dává vám informaci dopředu**, ne zpětně.

## Kdy začít testovat

Odečtěte od délky svého nejkratšího cyklu za poslední půlrok zhruba sedmnáct dní. Vyjde vám den, kdy je rozumné začít. U pravidelného osmadvacetidenního cyklu to znamená přibližně jedenáctý den.

Pokud je váš cyklus nepravidelný, začněte dřív a počítejte s tím, že testů spotřebujete víc.

## Jak testovat správně

Tady se dělá nejvíc chyb:

- **Netestujte z ranní moči.** LH se tvoří spíš dopoledne a v ranní moči nemusí být ještě zachytitelný. Obvykle se doporučuje testovat odpoledne, běžně mezi polednem a osmou večer.
- **Dvě hodiny před testem výrazně nepijte** a před testem se vymočte co nejméně často. Zředěná moč výsledek podhodnotí.
- **Testujte ve stejnou denní dobu.**
- **Zvažte dvakrát denně** v očekávaném období. Vzestup LH může být krátký a jednou denně se dá minout.
- **Odečtěte výsledek v čase uvedeném v návodu**, ne po hodině.

## Jak číst výsledek

U klasického proužkového testu je test pozitivní, když je **testovací čárka stejně tmavá nebo tmavší než kontrolní**. Slabá čárka pozitivní výsledek neznamená. Malé množství LH je v moči přítomné pořád.

Digitální testy tuhle nejistotu odstraňují a zobrazují symbol. Jsou dražší, ale pokud vás odečítání odstínů stresuje, stojí to za to.

## Co může výsledek zkreslit

- **PCOS.** Při syndromu polycystických ovarií bývá bazální hladina LH trvale vyšší a testy mohou vycházet pozitivně opakovaně, aniž by ovulace nastala. U PCOS jsou proto LH testy často zavádějící. Proberte s lékařem, co má u vás smysl.
- **Léky s obsahem hCG** používané v léčbě neplodnosti mohou výsledek ovlivnit.
- **Hormonální antikoncepce** v prvních cyklech po vysazení.
- **Perimenopauza** a stavy s trvale vyšším LH.
- **Velmi zředěná moč.**

## Pozitivní test: a co teď

Pozitivní LH test znamená, že plodné okno vrcholí. V praxi se doporučuje mít styk **v den pozitivního testu a den následující**, ideálně také den předtím, pokud jste ho zachytila.

Zároveň: **pozitivní test neznamená, že ovulace stoprocentně proběhla.** Znamená, že proběhl hormonální signál. Potvrzení dá vzestup bazální teploty nebo ultrazvuk.

## Kdy testy odložit

Pokud máte pravidelný cyklus a pravidelný sexuální život několikrát týdně, testy vlastně nepotřebujete. Plodné okno tak jako tak trefíte. Testy dávají největší smysl při:

- nepravidelném cyklu,
- vztahu na dálku nebo směnném provozu,
- snaze o co nejpřesnější načasování při omezených příležitostech.

## Kdy jít k lékaři

- Za několik po sobě jdoucích cyklů **nezachytíte pozitivní test** vůbec.
- Testy vycházejí pozitivně **mnoho dní v řadě**.
- Cyklus je nepravidelný nebo menstruace vynechává.

Nezachycený vzestup LH neznamená automaticky, že neovulujete. Může jít o technickou chybu v testování. Ale opakovaně to stojí za konzultaci.

> Tento článek nenahrazuje lékařskou péči. Domácí testy neslouží k diagnostice poruch ovulace. Tu stanoví lékař na základě vyšetření.`,
      minutes: 7,
      phases: ['trying_naturally'],
      dayRange: [0, 180],
      topics: ['hormony', 'vysledky'],
      level: 'essential',
      hero: 'pearl',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['ESHRE: doporučené postupy', 'Česká gynekologická a porodnická společnost ČLS JEP'],
      publishedOn: '2025-11-27',
      boost: 0.3,
    },
    {
      id: 'plan-cervikalni-hlen',
      kind: 'article',
      title: 'Cervikální hlen: nejlevnější ukazatel plodných dnů',
      excerpt:
        'Vaše tělo dává signál, který nepotřebuje baterie ani objednávku. Stačí ho umět přečíst.',
      body: `## Proč se tím vůbec zabývat

Hlen produkovaný hrdlem děložním se v průběhu cyklu mění pod vlivem estrogenu. V plodných dnech se **mění na řídký, průhledný a tažný**: právě takový, který spermiím umožní projít krčkem, vyživí je a udrží při životě až několik dní.

Mimo plodné dny je hlen naopak hustý a tvoří v krčku bariéru. Proto je změna hlenu pro plánování cennější než většina aplikací: **předpovídá plodné dny dopředu**, na rozdíl od bazální teploty.

## Jak hlen v cyklu vypadá

**Po menstruaci**: sucho nebo velmi málo hlenu, pocit spíš sušší.

**Několik dní poté**: hlen se objeví, ale je hustší, lepkavý, bělavý nebo krémový. Prsty se nedá roztáhnout, trhá se.

**Blíží se ovulace**: hlen řídne, je vodnatější, průhlednější, je ho víc.

**Vrchol plodnosti**: hlen připomíná **syrový vaječný bílek**: čirý, kluzký, tažný. Mezi prsty se roztáhne do několika centimetrů, aniž by se přetrhl. Ve spodním prádle působí vodnatě. Tenhle typ hlenu je nejlepším přirozeným signálem, že plodné okno je otevřené.

**Po ovulaci**: vlivem progesteronu hlen během jednoho až dvou dnů zhoustne, zežloutne nebo zmizí úplně.

## Jak ho sledovat, aniž by to bylo divné

Nemusíte nic zvláštního dělat. Stačí:

- **Všímat si při každé návštěvě toalety**: na papíře, ve spodním prádle.
- **Zapsat jedno slovo denně** do poznámek: sucho, lepkavý, krémový, vodnatý, bílek.
- **Sledovat i pocit**: v plodných dnech mívá spousta žen pocit vlhka, kluzkosti.

Nemusíte hlen odebírat prsty. Když chcete, udělejte to čistýma rukama, ale pozorování ze spodního prádla a papíru bohatě stačí.

## Co pozorování zkresluje

- **Pohlavní styk** a semenná tekutina, proto se hlen posuzuje spíš mimo den styku nebo s odstupem.
- **Lubrikanty**. Běžné lubrikanty mohou navíc negativně ovlivnit pohyblivost spermií, pokud lubrikant potřebujete, poraďte se v lékárně o variantě označené jako vhodná při snaze o početí.
- **Vaginální infekce.** Hlen s nepříjemným zápachem, výrazně žlutý, zelený, sýrovitý nebo doprovázený svěděním a pálením není plodný hlen. Je to důvod ke gynekologické kontrole.
- **Léky**, včetně některých antihistaminik, které mohou hlen vysoušet.
- **Nedostatek tekutin.**

## Když plodný hlen nepozorujete

Nejdřív klid: **rozpoznání typického hlenu se učí několik cyklů.** Spousta žen ho zpočátku přehlédne, protože čeká něco výraznějšího.

Pokud ale i po několika cyklech pozorování hlen typu vaječného bílku nikdy nevidíte, zmiňte to gynekologovi. Souviset to může s hormonálním nastavením, s obdobím po vysazení antikoncepce, s věkem, s předchozími zákroky na děložním hrdle nebo s užívanými léky.

## Kdy volat lékaře

Objednejte se na kontrolu, pokud pozorujete:

- výtok s nepříjemným zápachem, zelený nebo hnisavý,
- výtok doprovázený svěděním, pálením, bolestí při styku či močení,
- krvácení mimo menstruaci opakovaně,
- horečku spolu s bolestí v podbřišku. To patří k lékaři neodkladně.

## Jak si to shrnout

1. Zapisujte jedno slovo denně, nic víc.
2. Když se objeví kluzký, tažný, průhledný hlen, plodné okno je otevřené. A je to čas, kdy má styk největší smysl.
3. Kombinujte s ovulačním testem; ty dvě informace spolu dávají velmi dobrý obrázek.

> Tento text má informativní charakter a nenahrazuje gynekologické vyšetření. Změny výtoku, které vás znepokojují, vždy konzultujte s lékařem.`,
      minutes: 6,
      phases: ['trying_naturally'],
      dayRange: [0, 150],
      topics: ['hormony', 'sebepece'],
      level: 'deep',
      hero: 'blush',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['Česká gynekologická a porodnická společnost ČLS JEP', 'ESHRE: doporučené postupy'],
      publishedOn: '2025-12-04',
    },
    {
      id: 'plan-plodne-okno-nacasovani',
      kind: 'article',
      title: 'Plodné okno: šest dní, na kterých záleží',
      excerpt:
        'Vajíčko žije necelý den. Spermie až pět dní. Z toho vyplývá všechno, co potřebujete o načasování vědět.',
      body: `## Matematika plodnosti

Otěhotnět lze jen v omezené části cyklu a ta část je kratší, než většina lidí čeká.

- **Vajíčko** je po ovulaci schopné oplození zhruba **dvanáct až dvacet čtyři hodin**.
- **Spermie** dokážou v příznivém cervikálním hlenu přežít **až pět dní**.

Když ta dvě čísla složíte, vyjde vám **plodné okno o délce zhruba šesti dní**: pět dní před ovulací a den ovulace samotný. Nejvyšší pravděpodobnost otěhotnění mají obvykle dva až tři dny **před** ovulací a den ovulace.

Zásadní důsledek: **čekat s pohlavním stykem až na den ovulace je pozdní strategie.** Spermie mají být na místě dřív, než vajíčko dorazí, ne naopak.

## Kdy u vás ovulace nastává

Ovulace je zhruba **čtrnáct dní před další menstruací**, ne čtrnáct dní po té minulé. U cyklu délky 26 dní tedy padá kolem 12. dne, u cyklu délky 33 dní kolem 19. dne.

Pro určení plodného okna kombinujte:

- **délku svých posledních cyklů**: hrubý odhad,
- **cervikální hlen**: když se objeví kluzký, tažný hlen, okno je otevřené,
- **LH test**: pozitivní test znamená ovulaci zhruba do 24 až 36 hodin,
- **bazální teplotu**: zpětné potvrzení pro příští měsíc.

## Jak často

Tady se šíří dvě protichůdné pověry: „šetřit ho, ať je silnější" a „každý den, jinak to nevyjde". Ani jedna neodpovídá tomu, co se doporučuje.

Rozumný a v praxi nejčastěji doporučovaný přístup je **pohlavní styk každý druhý den nebo obden v průběhu plodného okna**, případně pravidelně dvakrát až třikrát týdně po celý cyklus. Druhá varianta má velkou výhodu: **nemusíte nic počítat** a plodné okno trefíte automaticky.

Dlouhá abstinence kvalitu spermatu nezlepšuje. Naopak. U mnoha mužů se při delší pauze zhoršuje pohyblivost spermií.

## Co na výsledek nemá vliv

Ušetřete si energii, tohle opravdu nerozhoduje:

- **Poloha při styku.** Žádná není „plodnější".
- **Ležení s nohama nahoře půl hodiny.** Spermie se do vejcovodů dostávají během minut a to, co vyteče, jsou převážně součásti semenné tekutiny, ne spermie. Pokud vám ležení dělá dobře, ležte, ale ne z povinnosti.
- **Orgasmus ženy.** Otěhotnění na něm nezávisí. Neškodí ale ani trochu.
- **Fáze měsíce, strava v ten den, myšlenky.**

Na co vliv naopak má: **lubrikanty**. Běžné lubrikanty mohou zhoršit pohyblivost spermií. Pokud lubrikant potřebujete, vyberte v lékárně ten, který je označený jako vhodný při snaze o početí.

## Kolik to trvá

Tohle číslo pomáhá víc než cokoli jiného: **i zcela zdravý pár má v jednom cyklu pravděpodobnost otěhotnění řádově kolem dvaceti až pětadvaceti procent.** Většina párů otěhotní během prvního roku, značná část z nich během prvního půlroku.

Znamená to, že tři neúspěšné cykly nejsou signál problému. Znamená to jen, že jste ve statistice, ve které se pohybují všichni. **Konkrétní pravděpodobnost pro vás ovlivňuje věk a případná diagnóza. A tu vám dokáže posoudit jen váš lékař.**

## Kdy plánování naopak škodí

Když se sex promění v úkol, klesá chuť a stoupá napětí. Pokud poznáváte tyhle věty, přehodnoťte strategii:

- „Dnes musíme, i když ani jeden nechce."
- „Bojím se, že to nezvládne, protože ví, který je den."
- Sex mimo plodné okno se úplně vytratil.

Varianta pravidelných dvou až tří styků týdně bez počítání dní bývá v takové situaci lepší volbou než přesné trefování termínu.

## Shrnutí na jednu kartičku

1. Plodné okno je zhruba šest dní a končí ovulací.
2. Nejlepší jsou dva až tři dny před ovulací.
3. Obden během okna, nebo prostě pravidelně 2–3× týdně.
4. Polohy, ležení a triky nic nemění.
5. Nezabralo to za tři měsíce? To je stále normální.

> Tento článek má informativní charakter a nenahrazuje lékařskou péči. Pokud se snažíte déle než rok (nebo déle než půl roku, je-li vám nad 35 let), objednejte se k lékaři.`,
      minutes: 8,
      phases: ['trying_naturally', 'preparing_body'],
      dayRange: [0, 200],
      topics: ['hormony', 'vztah', 'cekani'],
      level: 'essential',
      hero: 'champagne',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['ESHRE: doporučené postupy', 'Česká gynekologická a porodnická společnost ČLS JEP'],
      publishedOn: '2025-12-11',
      boost: 0.5,
    },
    {
      id: 'plan-myty-o-poceti',
      kind: 'article',
      title: 'Deset mýtů o početí, které vás jen zdržují',
      excerpt:
        'Od nohou nahoře po „hlavně se uvolni". Přehled toho, co si můžete s klidem přestat vyčítat.',
      body: `## Proč se tím zabývat

Mýty o početí nejsou neškodné. Kradou pozornost, vytvářejí pocit viny a někdy oddálí návštěvu lékaře o měsíce. Tohle je jejich seznam a to, co o nich skutečně víme.

## 1. „Stačí se uvolnit a přijde to samo."

Nejrozšířenější a nejzraňující věta v celé oblasti. Stres ovlivňuje cyklus a chuť na sex, to je pravda. Ale **neplodnost není způsobená tím, že se dost nesnažíte uvolnit.** Neprůchodné vejcovody, endometrióza ani nález na spermiogramu se relaxací nespraví. Pokud vám tuhle větu někdo řekne, není to rada. Je to jeho bezradnost.

## 2. „Poloha rozhoduje."

Nerozhoduje. Ani ležení s nohama nahoře po styku není podmínkou. Spermie se pohybují aktivně a do vejcovodů se dostávají velmi rychle.

## 3. „Musí se šetřit, aby měl víc spermií."

Delší abstinence sice zvýší objem, ale často zhorší pohyblivost. Pravidelný sexuální život je pro plodnost příznivější než pauzy.

## 4. „Antikoncepce po letech užívání snižuje plodnost."

Plodnost se po vysazení hormonální antikoncepce obvykle vrací během několika týdnů až měsíců. To, co se změnilo, je většinou **věk**: mezi začátkem užívání a vysazením uplynulo deset let. Pokud se ale cyklus neobnoví do několika měsíců, patří to k lékaři.

## 5. „Když mám pravidelný cyklus, ovuluji."

Pravidelný cyklus je dobrá zpráva a ovulaci **pravděpodobně** potvrzuje, ale ne jistě. Existují cykly bez ovulace, které vypadají pravidelně. Potvrzení dá teplotní křivka, hormonální odběr nebo ultrazvuk.

## 6. „Neplodnost je ženský problém."

Není. Faktor na straně muže se podílí zhruba v polovině případů. Proto do diagnostiky patří spermiogram hned na začátku. Je levný, rychlý a neinvazivní.

## 7. „Věk je jen číslo."

Bohužel není. Zásoba i kvalita vajíček se s věkem mění a od poloviny třicátých let výrazněji. To neznamená, že po pětatřiceti nelze otěhotnět. Znamená to jen, že **čekat na lepší chvíli má svou cenu**.

## 8. „Jedna káva to zkazí."

Nezkazí. Odborná doporučení hovoří o omezení kofeinu, ne o absolutním zákazu. Naproti tomu **u alkoholu je nejbezpečnější varianta žádný**, a to už ve fázi snažení, protože o prvních týdnech těhotenství zpravidla nevíte.

## 9. „Když jsem už jednou otěhotněla, půjde to zas."

Sekundární neplodnost je reálná a poměrně častá. Od minulého těhotenství uplynul čas, mohly přibýt srůsty, endometrióza nebo se změnily parametry na straně partnera. Předchozí těhotenství je dobrý signál, ale ne záruka. A nezakládá důvod odkládat vyšetření.

## 10. „Když si dám pauzu, přijde to."

Tenhle příběh znáte od kamarádky a je pravdivý. U ní. Statisticky ale platí, že páry s poruchou plodnosti pauzou nezískají nic než ztracený čas. Pauza má smysl kvůli **psychice**, a to je naprosto legitimní důvod. Jen ji neberte jako léčebnou metodu.

## Bonus: věci, které se šíří a nemají oporu

- Zásadité sprchování „na kluka" a další metody plánování pohlaví.
- Určování ovulace podle fáze měsíce.
- „Detox" před početím pomocí bylinných směsí. U řady z nich navíc nevíte, co obsahují.
- Rady typu „nesmíš se zvedat, nesmíš cvičit, nesmíš pít studené".

## Co dělat místo toho

1. Poznejte svůj cyklus.
2. Mějte pravidelný sexuální život, ne dokonale načasovaný.
3. Řešte alkohol, kouření a spánek u obou.
4. Dodržte časový limit pro návštěvu lékaře. Rok, nebo půl roku po pětatřicítce.

> Tento článek nenahrazuje lékařskou péči. Otázky ke své konkrétní situaci vždy směřujte na svého gynekologa.`,
      minutes: 7,
      phases: ['trying_naturally', 'preparing_body', 'thinking'],
      topics: ['psychika', 'hormony', 'komunita'],
      level: 'deep',
      hero: 'sand',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['ESHRE (doporučené postupy', 'NICE) doporučené postupy pro léčbu neplodnosti'],
      publishedOn: '2026-01-08',
    },
    {
      id: 'plan-kdy-vyhledat-lekare',
      kind: 'article',
      title: 'Kdy přestat čekat a jít k lékaři',
      excerpt:
        'Rok při věku do 35 let, půl roku od 35 let výš. A celá řada situací, kdy se nemá čekat vůbec.',
      body: `## Jednoduché pravidlo

Odborná doporučení se shodují na dvou hranicích:

- **Do 35 let: po dvanácti měsících** pravidelného nechráněného pohlavního styku bez otěhotnění.
- **Od 35 let: po šesti měsících.**
- **Od 40 let: neotálejte vůbec** a objednejte se, jakmile se rozhodnete pro dítě.

Tohle není trest ani nálepka. Je to jen doba, po které se považuje za rozumné zjistit, jestli něčemu nebrání konkrétní a řešitelný důvod.

## Kdy nečekat ani den z těch lhůt

Objednejte se **hned na začátku**, pokud se vás týká něco z tohoto seznamu:

- **cyklus kratší než 21 dní nebo delší než 35 dní**, výrazně nepravidelný nebo úplně chybějící,
- **silné, vyřazující bolesti při menstruaci** nebo bolestivý pohlavní styk (může jít o endometriózu),
- diagnostikované **PCOS**, endometrióza, adenomyóza,
- onemocnění **štítné žlázy**, cukrovka, autoimunitní onemocnění,
- **operace v malé pánvi** v anamnéze, zánět vejcovodů, mimoděložní těhotenství,
- **dva a více potratů** v anamnéze,
- **onkologická léčba** u vás nebo u partnera,
- známý **problém na straně partnera**: operace varlat, nesestouplé varle, úraz, příušnice v dospělosti, potíže s erekcí nebo ejakulací,
- v rodině **předčasné selhání vaječníků** nebo menopauza před čtyřicátým rokem.

## Ke komu jít první

Začněte u **svého gynekologa**. Ten provede základní vyšetření a v případě potřeby vás odešle na **centrum asistované reprodukce**. Ve většině situací je potřeba doporučení, ale spousta center přijímá i objednávky přímo. Zavolejte a zeptejte se.

Důležité: **na první vyšetření jděte oba.** Diagnostika páru bez spermiogramu je poloviční a zbytečně dlouhá.

## Co vás na začátku obvykle čeká

Přesnou skladbu určuje lékař, obecně se ale řeší:

- podrobná anamnéza obou partnerů,
- gynekologické vyšetření a **ultrazvuk** (počet antrálních folikulů, stav dělohy),
- **hormonální odběry** v konkrétní den cyklu, proto se termín odběru plánuje,
- **AMH** jako orientační ukazatel vaječníkové rezervy,
- vyšetření **štítné žlázy** a prolaktinu,
- **spermiogram** u partnera,
- vyšetření **průchodnosti vejcovodů**: obvykle až v dalším kroku,
- vyloučení infekcí.

Výsledky se skládají několik týdnů, protože část odběrů je vázaná na určitou fázi cyklu. Není to zdržování, je to nutnost.

## Jak si vyšetření usnadnit

1. **Přineste si zápisky o cyklu** za poslední tři měsíce.
2. **Sepište si otázky předem.** V ordinaci na ně zapomenete.
3. **Vezměte si starší nálezy**: operace, laboratorní výsledky, propouštěcí zprávy.
4. **Napište si seznam všech léků a doplňků**, které oba užíváte.
5. **Zeptejte se na časový plán.** Co, kdy a proč. A kdy je další kontrola.

## Věty, které mají v ordinaci zaznít

- „Jak dlouho bude trvat, než budeme mít kompletní obrázek?"
- „Co konkrétně z těchto výsledků vysvětluje naši situaci?"
- „Jaké jsou naše možnosti a v jakém pořadí je doporučujete?"
- „Co se stane, když ještě půl roku počkáme?"

## Když vám lékař řekne, ať to zkoušíte dál

Někdy je to správná rada. Někdy ale platí, že vaše obavy nejsou brané vážně. Pokud jste splnila časové kritérium, přesto je vám odmítnuto vyšetření, a nedostanete jasné zdůvodnění, **máte právo na druhý názor.** Vyhledat jiného gynekologa nebo centrum asistované reprodukce není nevděk, je to péče o vlastní čas.

## Kdy volat lékaře neodkladně

- silné krvácení mimo menstruaci,
- horečka spolu s bolestí v podbřišku,
- náhlá prudká bolest v podbřišku, zvlášť jednostranná,
- pozitivní těhotenský test spolu s bolestí a krvácením. Může jít o mimoděložní těhotenství a to je akutní stav.

> Tento článek má informativní charakter a nenahrazuje lékařskou péči. O rozsahu a načasování vyšetření vždy rozhoduje váš lékař.`,
      minutes: 9,
      phases: ['trying_naturally'],
      dayRange: [150, 400],
      topics: ['klinika', 'vysledky', 'partner'],
      level: 'essential',
      hero: 'sky',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: [
        'ESHRE: doporučené postupy',
        'NICE: doporučené postupy pro léčbu neplodnosti',
        'Sekce asistované reprodukce ČGPS ČLS JEP',
      ],
      publishedOn: '2026-01-15',
      boost: 0.5,
    },
    {
      id: 'plan-finance-pojistovna',
      kind: 'article',
      title: 'Peníze a pojišťovna: co v Česku platí stát a co vy',
      excerpt:
        'Základní diagnostika bývá hrazená, u léčby to začíná být složitější. A nejdražší je odkládání otázky.',
      body: `## Proč to řešit dřív, než to bude aktuální

Finanční stránka bývá poslední věc, kterou chcete řešit ve chvíli, kdy se dozvíte diagnózu. Právě proto se vyplatí mít hrubou představu předem, ne kvůli číslům, ale kvůli klidu.

**Důležité upozornění:** podmínky úhrady se v čase mění a liší se podle konkrétní situace i zdravotní pojišťovny. Nikde v tomhle textu proto nenajdete konkrétní částky ani počty cyklů jako fakt. **Aktuální podmínky vám vždy potvrdí vaše zdravotní pojišťovna a vaše klinika**: a je rozumné se na ně zeptat hned při první návštěvě.

## Co bývá hrazené z veřejného zdravotního pojištění

Obecně platí, že **diagnostika neplodnosti** je součástí běžné hrazené péče:

- gynekologické vyšetření a ultrazvuk,
- hormonální odběry,
- spermiogram,
- vyšetření průchodnosti vejcovodů,
- vyšetření štítné žlázy, infekcí a další indikovaná vyšetření.

U **léčby metodami asistované reprodukce** je situace jiná. V Česku je počet hrazených cyklů a věková hranice pacientky dán legislativou. **Konkrétní podmínky ověřte u své pojišťovny**: patří sem i to, za jakých okolností lze získat cykly navíc.

## Co se obvykle doplácí

I když je samotný cyklus hrazený, řada položek kolem něj hrazená být nemusí:

- **doplatky za léky** používané při stimulaci,
- některé **laboratorní metody** navíc,
- **preimplantační genetické testování (PGT)**,
- **kryokonzervace embryí** a její prodlužování v dalších letech,
- **nadstandardní služby**, konzultace mimo pojištění, expresní termíny,
- **dárcovské programy** a s nimi spojené poplatky.

Tenhle seznam je orientační. **Vyžádejte si od kliniky písemný ceník** a nechte si vysvětlit, které položky jsou volitelné a které nutné.

## Otázky, které se vyplatí položit na klinice

1. Které úkony v našem případě hradí pojišťovna a které ne?
2. Kolik se u nás obvykle doplácí za léky a od čeho se to odvíjí?
3. Jaké položky se mohou v průběhu cyklu přidat a proč?
4. Kolik stojí zamražení embryí a kolik jeho prodloužení každý další rok?
5. Co se stane s uhrazenou částkou, když se cyklus přeruší?
6. Máte písemný ceník, který si mohu vzít domů?

## Co ještě může finančně pomoci

- **Příspěvky zdravotních pojišťoven z fondu prevence.** Jednotlivé pojišťovny nabízejí různé programy, které se každý rok mění. Zeptejte se přímo na pobočce nebo v aplikaci své pojišťovny.
- **Příspěvky od zaměstnavatele**: v některých firmách existují benefity na zdravotní péči, které lze použít i tímto směrem.
- **Daňové odpočty** za zdravotní výdaje mají svá pravidla; ověřte si aktuální stav u daňového poradce.
- **Přeshraniční péče**: pokud zvažujete léčbu v zahraničí, ověřte si podmínky úhrady předem, protože se výrazně liší.

## Praktické rady, které ušetří nervy

- **Založte si jednu složku** (fyzickou i v telefonu) na všechny účtenky, faktury a zprávy. Budete je potřebovat víc, než čekáte.
- **Nastavte si rozpočet na rok, ne na cyklus.** Léčba je běh na delší trať a jednorázová čísla klamou.
- **Domluvte se s partnerem, odkud peníze půjdou** a jaký je váš strop. Ne proto, abyste se omezovali, ale abyste se o tom nemuseli hádat ve chvíli, kdy budete oba vyčerpaní.
- **Neberte si úvěr pod tlakem času.** Rozhodnutí o penězích dělejte v klidný den, ne den po neúspěšném cyklu.

## A jedna věc, o které se nemluví

Náklady nejsou jen finanční. Jsou to i dovolené vyčerpané na odběry, dojíždění, neschopenky, hodiny čekání. Připočtěte je do svých úvah, ne abyste se lekla, ale abyste si dokázala naplánovat rezervu a nevyčítala si, že to zvládáte hůř, než jste čekala.

> Tento text má informativní charakter, nejde o právní ani finanční poradenství a nenahrazuje lékařskou péči. Podmínky úhrady si vždy ověřte u své zdravotní pojišťovny a kliniky.`,
      minutes: 8,
      phases: ['thinking', 'trying_naturally'],
      dayRange: [180, 400],
      topics: ['finance', 'klinika'],
      level: 'deep',
      hero: 'linen',
      author: 'Tým BlooMia',
      sources: [
        'Zdravotní pojišťovny ČR. Podmínky úhrady',
        'Sekce asistované reprodukce ČGPS ČLS JEP',
      ],
      publishedOn: '2026-01-22',
    },
    {
      id: 'plan-sex-na-povel',
      kind: 'article',
      title: 'Když se sex stane úkolem: jak si ho vzít zpátky',
      excerpt:
        'Plánování početí umí ze vztahu udělat provoz. Tady je návod, jak z toho ven. Pro oba.',
      body: `## Jak se to stane

Nikdo si neřekne „od zítřka bude sex povinnost". Stane se to postupně. Nejdřív si jen poznamenáte, který je den cyklu. Pak přijde ovulační test. Pak věta „dnes musíme". A za pár měsíců se přistihnete, jak sledujete jeho reakci, když mu oznámíte, že je ten den.

Tohle je jeden z nejčastějších a nejméně sdílených problémů párů, které se snaží o dítě. Není to selhání vaší intimity. Je to logický důsledek toho, že jste do postele pustili kalendář.

## Co to dělá jemu

Muži v téhle situaci často zažívají něco, o čem nemluví: **tlak na výkon**. Vědomí, že „dnes to musí vyjít", je jeden z nejspolehlivějších způsobů, jak způsobit potíže s erekcí u zcela zdravého muže.

Když se to stane, obvykle následuje ticho a stud, případně vysvětlování typu „jsem unavený". Nejhorší, co se v tu chvíli dá udělat, je řešit to hned a věcně. Nejlepší je nechat to být a vrátit se k tomu jindy, s odstupem a bez obviňování.

## Co to dělá vám

Vy zase často zjistíte, že chuť zmizela úplně. Že se přistihnete, jak počítáte minuty. Že jste po styku smutná místo spokojená, protože už se v hlavě posouváte k testu.

I to je normální. A i to má řešení.

## Sedm věcí, které fungují

### 1. Zvolte strategii bez počítání
Místo přesného trefování ovulace zkuste **pravidelně dvakrát až třikrát týdně** po celý cyklus. Plodné okno trefíte tak jako tak a odpadne oznamování termínu.

### 2. Neříkejte mu, který je den
Vážně. Pokud to zvládnete, nechte si informaci o cyklu pro sebe a jen navrhněte večer spolu. Většina mužů funguje mnohem lépe bez datového vstupu.

### 3. Zaveďte sex, který není o početí
Pravidlo, které pomáhá: **jednou týdně blízkost, u které je předem jasné, že nesměřuje k otěhotnění.** Může to být cokoli. Od objímání po sex mimo plodné okno. Jde o to obnovit spojení, které není nástrojem.

### 4. Vraťte zpátky předehru
V režimu úkolu se z předehry stane formalita. Přitom je to přesně to, co dělá rozdíl mezi „museli jsme" a „bylo nám spolu dobře".

### 5. Změňte prostředí a čas
Ráno místo večera. Pátek místo úterý. Jinde než v ložnici, kde visí kalendář.

### 6. Mluvte o tom mimo postel
Ne v tu chvíli. Druhý den u kávy. Věta, která otevírá: „Chybíš mi. Ne ten sex. Ty."

### 7. Dejte si vědomou pauzu
Jeden cyklus bez měření, bez testů, bez počítání. Neztratíte tím nic zásadního a získáte hodně.

## Kdy vyhledat pomoc

Sexuolog nebo párový terapeut má smysl, když:

- potíže s erekcí nebo ejakulací se opakují i mimo plodné dny,
- jeden z vás se sexu systematicky vyhýbá déle než několik týdnů,
- pohlavní styk bolí. **bolestivý styk je vždy důvod ke gynekologickému vyšetření**, může za ním být endometrióza, infekce nebo jiná léčitelná příčina,
- téma sexu vyvolává hádky pokaždé.

Návštěva odborníka není přiznání porážky. Je to nejrychlejší cesta z bludného kruhu, ve kterém spousta párů uvízne na roky.

## Věta na závěr

Vy dva jste tady byli dřív než tenhle projekt. Ať to dopadne jakkoli, budete tady spolu i potom. Chránit vztah není odbočka z cesty za dítětem. Je to její součást.

> Tento text nenahrazuje odbornou péči. Bolest při pohlavním styku, opakované potíže s erekcí či dlouhodobá ztráta chuti patří k lékaři.`,
      minutes: 7,
      phases: ['trying_naturally'],
      dayRange: [60, 400],
      topics: ['vztah', 'partner', 'psychika'],
      level: 'essential',
      hero: 'dusk',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      publishedOn: '2026-02-05',
      boost: 0.3,
    },
    {
      id: 'plan-psychika-prvnich-mesicu',
      kind: 'article',
      title: 'Když přijde menstruace znovu: psychika prvních měsíců snažení',
      excerpt:
        'Nikdo vám neřekl, že to bude bolet už po třetím měsíci. A přitom to tak má většina žen.',
      body: `## To, co nikdo neřekl dopředu

Když jste vysadila antikoncepci, možná jste čekala nervozitu z toho, jestli to nepřijde moc rychle. Málokdo počítá s opakem. S tím, že třetí, pátý, osmý měsíc přinese menstruaci a s ní pokaždé stejné bodnutí.

Tohle není přecitlivělost. Každý cyklus má svůj oblouk: naděje kolem ovulace, napjaté čekání, sledování každého píchnutí v prsou, pak buď test, nebo krev. Ten oblouk se opakuje **dvanáctkrát do roka** a vyčerpává i ženy, které jsou jinak naprosto v pohodě.

## Proč to bolí, i když „se nic neděje"

Protože každý měsíc něco skutečně končí. Ne těhotenství, ale konkrétní představa. Ta, ve které jste si počítala, kdy by se dítě narodilo, komu byste to řekla první a jak by to bylo o Vánocích.

Truchlení nad představou je legitimní. Nemusíte ho zdůvodňovat a nemusíte ho srovnávat s lidmi, kteří to mají „horší".

## Co pomáhá, aniž by to znělo jako fráze

### Oddělte den D od zbytku měsíce
Den, kdy přijde menstruace, si předem ošetřete. Nedávejte si na něj náročné schůzky, pokud to jde. Domluvte se s partnerem, co v ten den potřebujete. Mlčení, objetí, nebo úplně obyčejný večer.

### Zmenšete okno čekání
Nejhorší nejsou dva týdny čekání. Nejhorší je testování od osmého dne. **Čím dřív testujete, tím víckrát prožijete zklamání v jednom cyklu.** Stanovte si jedno datum a držte se ho.

### Vyberte si, kdo to bude vědět
Nemusíte to říct nikomu, ale úplná samota to zhoršuje. Jeden člověk, který ví a nekomentuje, je obvykle přesně tak akorát.

### Připravte si odpovědi předem
Na „a kdy vy dva?" i na těhotenskou novinku od kamarádky. Předem promyšlená věta vás ochrání víc než odhodlání to zvládnout.
Například: „Řešíme to a nechci o tom teď mluvit, děkuju."

### Nechte si jednu věc, která není o dítěti
Práce, sport, kurz, cokoli, co má vlastní kalendář a vlastní úspěchy. Když celý život běží v rytmu cyklu, každý neúspěch je totální.

## Co nepomáhá, i když to všichni říkají

- „Uvolni se a přijde to." Neplodnost není nedostatek relaxace.
- „Zkuste na to nemyslet." Nedá se to a neměla byste to muset.
- „Vezmi si dovolenou, kamarádce to pomohlo." Někdy ano, ale jako recept to nefunguje.
- „Aspoň víš, že můžeš otěhotnět." U žen po ztrátě je tahle věta obzvlášť zraňující.

## Kdy už to není jen smutek

Vyhledejte psychologa nebo svého lékaře, pokud:

- smutek nebo úzkost trvají většinu dní déle než dva týdny,
- nemůžete spát nebo naopak nemůžete vstát,
- ztratila jste zájem o věci, které vás dřív bavily,
- objevují se panické stavy,
- máte myšlenky, že by bylo lepší tu nebýt. **v takovém případě vyhledejte pomoc okamžitě**, například na krizové lince, u praktického lékaře nebo na psychiatrické pohotovosti.

Psychologická podpora v období snažení není nadstandard. Řada center asistované reprodukce má vlastního psychologa a poradenství se dá najít i mimo ně.

## Jedna praktická věc na dnešek

Napište si někam, kde to zase najdete, tři věty:

1. Co v tomhle měsíci zvládám dobře.
2. Co je moje a co je biologie.
3. Koho poprosím o pomoc, až bude nejhůř.

Až přijde ten den, budete to mít připravené od člověka, který vás zná nejlíp.

> Tento text nenahrazuje odbornou psychologickou ani lékařskou péči. Pokud vaše obtíže trvají nebo se zhoršují, obraťte se na odborníka.`,
      minutes: 7,
      phases: ['trying_naturally'],
      dayRange: [60, 400],
      topics: ['psychika', 'sebepece', 'cekani'],
      level: 'comfort',
      hero: 'dawn',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      publishedOn: '2026-02-12',
      boost: 0.35,
    },
    {
      id: 'plan-checklist-90-dni-priprava',
      kind: 'checklist',
      title: 'Devadesát dní přípravy: checklist krok za krokem',
      excerpt:
        'Všechno, co má smysl stihnout tři měsíce před početím, rozdělené tak, aby to nebylo najednou.',
      body: `## Jak s tímhle seznamem pracovat

Není to soutěž a nemusíte odškrtat všechno. Projděte si položky, vyberte ty, které se vás týkají, a rozložte si je do tří měsíců. Nesplněná položka není selhání. Je to jen věc, která zatím nepřišla na řadu.

Body označené jako nepovinné jsou právě tím: nepovinné.

**Tento checklist má informativní charakter a nenahrazuje péči vašeho lékaře.** O doplňcích, lécích a vyšetřeních vždy rozhodujte společně s ním.

## Rozvržení, které se osvědčilo

- **První měsíc:** zdravotní základ. Objednávky, odběry, doplňky, alkohol.
- **Druhý měsíc:** návyky. Spánek, pohyb, nikotin, jídelníček.
- **Třetí měsíc:** cyklus a partner. Pozorování, zápisky, jeho příprava.

Pokud vám je nad pětatřicet nebo máte nepravidelný cyklus, nečekejte na pořadí a objednejte se k lékaři hned.`,
      minutes: 6,
      phases: ['preparing_body', 'thinking'],
      dayRange: [0, 90],
      topics: ['strava', 'sebepece', 'klinika', 'partner'],
      level: 'essential',
      hero: 'sage',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      publishedOn: '2025-10-30',
      boost: 0.4,
      checklist: [
        {
          id: 'plan-chk90-gyn',
          text: 'Objednat se na předkoncepční prohlídku ke gynekologovi',
          hint: 'Řekněte při objednání, že plánujete těhotenství. Dostanete delší termín.',
          group: 'Zdravotní základ',
        },
        {
          id: 'plan-chk90-cytologie',
          text: 'Zkontrolovat, kdy jsem měla naposledy cytologii',
          group: 'Zdravotní základ',
        },
        {
          id: 'plan-chk90-odbery',
          text: 'Domluvit s lékařem odběry: krevní obraz, feritin, štítná žláza, vitamin D',
          hint: 'Rozsah určuje lékař podle vaší anamnézy.',
          group: 'Zdravotní základ',
        },
        {
          id: 'plan-chk90-ockovani',
          text: 'Ověřit imunitu proti zarděnkám a planým neštovicím',
          hint: 'Živé vakcíny se v těhotenství nepodávají, proto se řeší předem.',
          group: 'Zdravotní základ',
        },
        {
          id: 'plan-chk90-leky',
          text: 'Ukázat lékaři seznam všech léků a doplňků, které užívám',
          hint: 'Nic nevysazujte sama, jen o tom dejte vědět.',
          group: 'Zdravotní základ',
        },
        {
          id: 'plan-chk90-folat',
          text: 'Začít užívat folát podle doporučení lékaře',
          group: 'Zdravotní základ',
        },
        {
          id: 'plan-chk90-zubar',
          text: 'Zajít na preventivní prohlídku k zubaři',
          group: 'Zdravotní základ',
        },
        {
          id: 'plan-chk90-chronicke',
          text: 'Zkonzultovat chronické onemocnění se svým specialistou',
          hint: 'Štítná žláza, cukrovka, autoimunitní onemocnění, epilepsie, psychiatrická medikace.',
          group: 'Zdravotní základ',
          optional: true,
        },
        {
          id: 'plan-chk90-alkohol',
          text: 'Vysadit alkohol',
          hint: 'Nejbezpečnější množství při snaze o dítě je žádné.',
          group: 'Návyky',
        },
        {
          id: 'plan-chk90-nikotin',
          text: 'Stanovit konkrétní datum posledního dne kouření (u obou)',
          group: 'Návyky',
        },
        {
          id: 'plan-chk90-kofein',
          text: 'Spočítat si všechny zdroje kofeinu za den a probrat míru s lékařem',
          group: 'Návyky',
        },
        {
          id: 'plan-chk90-spanek',
          text: 'Nastavit stejný čas vstávání sedm dní v týdnu',
          group: 'Návyky',
        },
        {
          id: 'plan-chk90-pohyb',
          text: 'Zavést tři procházky týdně, pevně v kalendáři',
          group: 'Návyky',
        },
        {
          id: 'plan-chk90-jidlo',
          text: 'Přidat bílkovinu a zeleninu do každého hlavního jídla',
          group: 'Návyky',
          optional: true,
        },
        {
          id: 'plan-chk90-cyklus',
          text: 'Zapsat první den poslední menstruace a nastavit připomínku na příští',
          group: 'Cyklus',
        },
        {
          id: 'plan-chk90-hlen',
          text: 'Začít si všímat charakteru cervikálního hlenu. Jedno slovo denně',
          group: 'Cyklus',
          optional: true,
        },
        {
          id: 'plan-chk90-teplomer',
          text: 'Pořídit digitální teploměr, pokud chci měřit bazální teplotu',
          group: 'Cyklus',
          optional: true,
        },
        {
          id: 'plan-chk90-partner-text',
          text: 'Poslat partnerovi článek o mužské přípravě',
          group: 'Partner',
        },
        {
          id: 'plan-chk90-partner-teplo',
          text: 'Domluvit s partnerem omezení sauny, horkých koupelí a notebooku na klíně',
          group: 'Partner',
        },
        {
          id: 'plan-chk90-role',
          text: 'Rozdělit si s partnerem role: kdo objednává termíny, kdo co hlídá',
          group: 'Partner',
        },
        {
          id: 'plan-chk90-slozka',
          text: 'Založit jednu složku na všechny nálezy a účtenky',
          group: 'Organizace',
        },
        {
          id: 'plan-chk90-otazky',
          text: 'Sepsat si otázky na lékaře do telefonu',
          group: 'Organizace',
        },
      ],
    },
    {
      id: 'plan-checklist-pro-partnera',
      kind: 'checklist',
      title: 'Checklist pro partnera: co může udělat on',
      excerpt:
        'Konkrétní seznam pro muže, který chce pomoct, ale neví jak. Pošlete mu ho, nevysvětlujte.',
      body: `## Pro toho, kdo tenhle seznam dostal

Tvoje partnerka ti poslala checklist. Neznamená to, že něco děláš špatně. Znamená to, že příprava na dítě není jen její věc a ty v ní máš vlastní, konkrétní podíl.

Dvě čísla, která stojí za zapamatování:

- **Faktor na straně muže se podílí zhruba v polovině případů neplodnosti.**
- **Tvorba spermií trvá zhruba tři měsíce.** Co změníš dnes, se projeví přibližně za devadesát dní.

To je vlastně dobrá zpráva. Tvoje část má rychlou návratnost a většina položek níž je jednoduchá.

**Tento seznam je informativní a nenahrazuje lékařskou péči.** O vyšetřeních a lécích se vždy poraď s lékařem.`,
      minutes: 5,
      phases: ['preparing_body', 'trying_naturally', 'thinking'],
      dayRange: [0, 120],
      topics: ['partner', 'vztah', 'strava'],
      level: 'essential',
      hero: 'taupe',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      publishedOn: '2025-12-18',
      checklist: [
        {
          id: 'plan-chkm-teplo',
          text: 'Omezit saunu, horké koupele a vyhřívanou sedačku v autě',
          hint: 'Varlata potřebují nižší teplotu než zbytek těla.',
          group: 'Rychlé změny',
        },
        {
          id: 'plan-chkm-notebook',
          text: 'Přestat dávat notebook na klín',
          group: 'Rychlé změny',
        },
        {
          id: 'plan-chkm-pradlo',
          text: 'Vyměnit těsné spodní prádlo a kalhoty za volnější',
          group: 'Rychlé změny',
          optional: true,
        },
        {
          id: 'plan-chkm-koureni',
          text: 'Přestat kouřit. Včetně zahřívaného tabáku a elektronických cigaret',
          hint: 'Zhoršuje počet, pohyblivost i tvar spermií. Platí i marihuana.',
          group: 'Životospráva',
        },
        {
          id: 'plan-chkm-alkohol',
          text: 'Výrazně omezit alkohol, ideálně na nulu',
          group: 'Životospráva',
        },
        {
          id: 'plan-chkm-strava',
          text: 'Přidat zeleninu, ryby, ořechy a olivový olej',
          group: 'Životospráva',
        },
        {
          id: 'plan-chkm-pohyb',
          text: 'Hýbat se aspoň třikrát týdně',
          group: 'Životospráva',
        },
        {
          id: 'plan-chkm-spanek',
          text: 'Spát alespoň sedm hodin a chodit spát v podobnou dobu',
          group: 'Životospráva',
          optional: true,
        },
        {
          id: 'plan-chkm-anabolika',
          text: 'Říct lékaři, pokud jsem někdy užíval testosteron nebo anabolika',
          hint: 'Zásadní informace. Může být klíčem k celé diagnóze. Nikdy nevysazovat bez lékaře.',
          group: 'Zdraví',
        },
        {
          id: 'plan-chkm-leky',
          text: 'Ukázat lékaři seznam léků a doplňků, které užívám',
          group: 'Zdraví',
        },
        {
          id: 'plan-chkm-spermiogram',
          text: 'Objednat se na spermiogram',
          hint: 'Levné, rychlé a neinvazivní. Nemá smysl s ním čekat rok.',
          group: 'Zdraví',
        },
        {
          id: 'plan-chkm-urolog',
          text: 'Zmínit lékaři operace varlat, úrazy, příušnice v dospělosti nebo varikokélu',
          group: 'Zdraví',
          optional: true,
        },
        {
          id: 'plan-chkm-termin',
          text: 'Vzít si na starost objednávání jednoho z termínů',
          hint: 'Konkrétní odlehčení, kterého si všimne víc než slov.',
          group: 'Vztah',
        },
        {
          id: 'plan-chkm-den-d',
          text: 'Domluvit se, co potřebuje v den, kdy přijde menstruace',
          group: 'Vztah',
        },
        {
          id: 'plan-chkm-sex',
          text: 'Domluvit se na blízkosti, která není o početí. Aspoň jednou týdně',
          group: 'Vztah',
        },
        {
          id: 'plan-chkm-fraze',
          text: 'Vyškrtnout z repertoáru větu „hlavně se uvolni"',
          hint: 'Nefunguje a zraňuje. Místo toho: „Jsem s tebou."',
          group: 'Vztah',
        },
      ],
    },
    {
      id: 'plan-checklist-prvni-navsteva',
      kind: 'checklist',
      title: 'První návštěva u lékaře kvůli plodnosti: co si vzít a na co se ptát',
      excerpt:
        'Připravená pacientka ušetří měsíc čekání. Tady je všechno, co si vzít s sebou a na co se zeptat.',
      body: `## Než tam půjdete

První konzultace bývá krátká a nabitá informacemi. Většina žen z ní odchází s pocitem, že zapomněla zeptat se na to hlavní. Tenhle seznam tomu má zabránit.

Dvě doporučení nad rámec odrážek:

1. **Jděte tam oba.** Diagnostika páru bez partnera je poloviční a zdrží vás o týdny.
2. **Pište si.** Během hovoru si dělejte poznámky, nebo si po domluvě s lékařem nahrávejte zvuk. Za dva dny si polovinu nebudete pamatovat.

**Tento seznam nenahrazuje lékařskou péči.** Rozsah vyšetření i další postup určuje váš lékař.`,
      minutes: 5,
      phases: ['trying_naturally'],
      dayRange: [150, 500],
      topics: ['klinika', 'vysledky', 'partner'],
      level: 'essential',
      hero: 'sky',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      publishedOn: '2026-01-29',
      boost: 0.3,
      checklist: [
        {
          id: 'plan-chk1-zaznamy',
          text: 'Zápisky o cyklu za poslední tři měsíce',
          hint: 'První dny menstruace, délka, bolesti, charakter hlenu.',
          group: 'Co si vzít',
        },
        {
          id: 'plan-chk1-nalezy',
          text: 'Starší lékařské zprávy a výsledky odběrů',
          group: 'Co si vzít',
        },
        {
          id: 'plan-chk1-operace',
          text: 'Informace o prodělaných operacích v malé pánvi a břiše',
          group: 'Co si vzít',
        },
        {
          id: 'plan-chk1-leky',
          text: 'Seznam léků a doplňků obou partnerů',
          group: 'Co si vzít',
        },
        {
          id: 'plan-chk1-prukazka',
          text: 'Kartička pojišťovny a doporučení od gynekologa, pokud je potřeba',
          group: 'Co si vzít',
        },
        {
          id: 'plan-chk1-partner',
          text: 'Partnera. Fyzicky, ne jen jeho výsledky',
          group: 'Co si vzít',
        },
        {
          id: 'plan-chk1-doba',
          text: 'Zeptat se: jak dlouho bude trvat, než budeme mít kompletní obrázek?',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-vysetreni',
          text: 'Zeptat se: která vyšetření nás čekají a v jakém pořadí?',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-cyklus',
          text: 'Zeptat se: která vyšetření jsou vázaná na konkrétní den cyklu?',
          hint: 'Zásadní pro plánování. Část odběrů se dělá jen v určité dny.',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-spermio',
          text: 'Zeptat se: kdy a kde se objedná spermiogram a jak se na něj připravit?',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-moznosti',
          text: 'Zeptat se: jaké jsou v naší situaci možnosti a co doporučujete jako první krok?',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-cekani',
          text: 'Zeptat se: co se stane, když ještě půl roku počkáme?',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-uhrada',
          text: 'Zeptat se: co hradí pojišťovna a co se doplácí?',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-kontakt',
          text: 'Zeptat se: kam volat, když se něco změní nebo budu mít dotaz?',
          group: 'Otázky',
        },
        {
          id: 'plan-chk1-kontrola',
          text: 'Odejít s konkrétním termínem další kontroly',
          group: 'Po návštěvě',
        },
        {
          id: 'plan-chk1-zapis',
          text: 'Hned po odchodu si zapsat, co zaznělo, dokud to mám v hlavě',
          group: 'Po návštěvě',
        },
        {
          id: 'plan-chk1-nazor',
          text: 'Zvážit druhý názor, pokud jsem nedostala jasné zdůvodnění',
          group: 'Po návštěvě',
          optional: true,
        },
      ],
    },
    {
      id: 'plan-plodne-okno',
      kind: 'article',
      title: 'Plodné okno: co většina z nás počítá špatně',
      excerpt: 'Po tomhle článku už nikdy nebudete počítat ovulaci od poslední menstruace.',
      body: `## Dvě čísla, ze kterých plyne všechno ostatní

Vajíčko je po ovulaci schopné oplození zhruba **dvanáct až dvacet čtyři hodin**.
Spermie přežívají v příznivém cervikálním hlenu **až pět dní**.

Z těchhle dvou čísel vzniká plodné okno dlouhé zhruba šest dní. A hlavně:
tohle okno **končí ovulací**, nezačíná jí. Většina párů, která to načasuje
špatně, to načasuje pozdě.

## Nejčastější chyba: čtrnáctý den

Pravidlo „ovulace je čtrnáctý den“ platí jen pro cyklus dlouhý 28 dní.

Představte si dva cykly vedle sebe. Jeden 28denní, druhý 33denní. U obou
trvá druhá fáze cyklu, tedy doba od ovulace do menstruace, zhruba stejně:
kolem čtrnácti dní. Ta délka je poměrně stálá. Co se mezi ženami a mezi
cykly liší, je první fáze, tedy doba od menstruace k ovulaci.

Proto se ovulace nepočítá od poslední menstruace dopředu, ale **od očekávané
další menstruace zpět**. U 33denního cyklu tak vychází zhruba na 19. den, ne
na 14.

Když máte cyklus nepravidelný, tohle počítání spolehlivě nefunguje vůbec
a je potřeba se řídit signály.

## Tři signály a jejich načasování

| Signál | Co znamená | Kdy přichází |
| --- | --- | --- |
| Cervikální hlen | Mění se na kluzký, tažný, průhledný | Několik dní **před** ovulací |
| LH test | Zachytí vzestup luteinizačního hormonu | Zhruba 24 až 36 hodin **před** ovulací |
| Bazální teplota | Vzestup o dvě až pět desetin stupně | **Až po** ovulaci |

Z toho plyne důležitá věc: teplota přichází pozdě na to, aby se podle ní dal
v témže cyklu načasovat styk. Slouží ke zpětnému potvrzení, ne k plánování.

## Dvě strategie, obě v pořádku

1. **Styk obden během plodného okna.** Přesnější, ale znamená to počítat
   a sledovat.
2. **Pravidelně dvakrát až třikrát týdně po celý cyklus, bez počítání.**
   Statisticky srovnatelné a pro vztah obvykle šetrnější, protože se ze
   sexu nestane úkol s termínem.

Pokud vám první varianta bere víc, než dává, druhá není horší volba.

## Co na výsledek nemá vliv

- Poloha při styku.
- Ležení s nohama nahoře po něm.
- Denní doba.

## A jedna věc, která vliv mít může

Běžné lubrikanty mohou zhoršit pohyblivost spermií. Existují lubrikanty
označené jako vhodné při snaze o početí.

> Článek má informativní charakter a nenahrazuje lékařskou péči. Pokud se
> snažíte déle než rok, nebo déle než půl roku při věku nad 35 let,
> objednejte se k lékaři.`,
      minutes: 6,
      phases: ['trying_naturally', 'preparing_body'],
      dayRange: [0, 200],
      topics: ['hormony', 'cekani', 'vztah'],
      level: 'essential',
      hero: 'champagne',
      author: 'Gabi',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      sources: ['ESHRE: doporučené postupy'],
      publishedOn: '2026-02-19',
    },
    {
      id: 'plan-jak-merit-bazalni-teplotu',
      kind: 'article',
      title: 'Bazální teplota: jak měřit a jak číst graf',
      excerpt: 'Návod pro ženy, které nechtějí strávit tři cykly hledáním, co dělají špatně.',
      body: `## Čím a kdy měřit

Digitální teploměr **se dvěma desetinnými místy**. Rozdíly, které hledáte,
jsou v desetinách stupně, takže teploměr s jedním desetinným místem je na to
příliš hrubý.

Pravidlo měření:

- **hned po probuzení**, před vstáváním a napitím,
- **ve stejnou dobu**, plus minus půl hodiny,
- **po alespoň třech hodinách souvislého spánku**.

A jedno pravidlo navíc: metodu měření nemíchejte. Kdo měří vaginálně, měří
vaginálně celý cyklus. Přechod mezi ústy a pochvou uprostřed cyklu udělá
v grafu skok, který vypadá jako ovulace a není.

## Jak vypadá graf, ve kterém ovulace proběhla

Po ovulaci začne žluté tělísko tvořit progesteron a ten mírně zvyšuje
tělesnou teplotu.

V grafu to vypadá takhle:

1. **První fáze cyklu:** nižší hodnoty, kolísají v úzkém pásmu.
2. **Skok:** o dvě až pět desetin stupně, obvykle během jednoho až dvou dní.
3. **Druhá fáze:** stabilně vyšší hladina, drží se až do menstruace.

Tomuhle se říká **dvoufázová křivka** a znamená, že ovulace nejspíš proběhla.
Plochá křivka bez skoku může znamenat cyklus bez ovulace, ale taky
nepřesné měření.

## Co graf neumí

Tohle je nejdůležitější část celého článku.

Teplota stoupá **až po ovulaci**. V daném cyklu už tedy na načasování styku
nepomůže. Slouží ke zpětnému potvrzení a k poznání vlastního vzorce napříč
cykly.

A druhá věc: tvar křivky **není diagnóza**. Krátká druhá fáze je informace
pro lékaře, ne závěr, který si uděláte sama v tři ráno nad diskusním fórem.

## Dny, které se do hodnocení nepočítají

Teplotu spolehlivě rozhodí:

- nemoc nebo zvýšená teplota,
- alkohol předchozí večer,
- špatně prospaná noc nebo měření po probuzení v jinou dobu,
- cesta přes časová pásma.

Tyhle dny si označte a při hodnocení je vyloučte. Jeden vyskočený bod
neznamená nic.

## Poslední věc, a je nejdůležitější

**Měření je nástroj, ne povinnost.** Pokud vám bere víc klidu, než dává
informace, je úplně v pořádku ho odložit.

> Článek je informativní a nenahrazuje lékařské vyšetření. Grafy teploty
> posuzuje lékař v kontextu dalších nálezů.`,
      minutes: 7,
      phases: ['trying_naturally'],
      dayRange: [0, 150],
      topics: ['hormony', 'vysledky'],
      level: 'deep',
      hero: 'dawn',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      publishedOn: '2026-03-05',
    },
    {
      id: 'plan-vecerni-uvolneni',
      kind: 'article',
      title: 'Večer, kdy zase nic: dvanáct minut na uklidnění',
      excerpt:
        'Vedené zklidnění pro den, kdy přišla menstruace a vy potřebujete něco jiného než dobré rady.',
      body: `Tohle není cvičení, které vám bude tvrdit, že všechno bude dobré. Je to
dvanáct minut pro večer, kdy jste zjistila, že tenhle cyklus zase nevyšel.

Můžete si to přečíst očima, nebo si to nechat přečíst nahlas. Nemusíte projít
celé.

## 1. Přistání

Žádné dýchání do břicha, žádné vizualizace. Zatím nic.

To, co teď cítíte, dává smysl. Nemusíte to zlehčovat ani zveličovat.

Položte si ruku někam, kde ji budete cítit. Na hrudník, na břicho, na stehno.
Nechte ji tam.

## 2. Dech

Nádech nosem na čtyři doby. Výdech ústy na šest.

Ještě jednou. Nádech čtyři. Výdech šest.

Delší výdech aktivuje tlumivou část nervového systému. Je to jedna z mála
věcí, které fungují spolehlivě a hned. Není to trik, je to fyziologie.

Zůstaňte u toho pár minut, ve svém tempu. Nemusíte počítat přesně.

## 3. Uvolnění těla

Postupně povolte:

- **čelist**, zuby se nemusí dotýkat,
- **ramena**, nechte je klesnout,
- **břicho**, měkké,
- **pánevní dno**, tohle místo drží napětí u žen, které měsíce čekají na
  výsledek, a skoro nikdo o něm nemluví.

## 4. Dvě věty na konec

Nejsou to afirmace o pozitivním myšlení.

**Tohle byl těžký den a vy jste ho zvládla.**

**Zítra po vás nikdo nechce, abyste byla v pořádku.**

## Kdy se k tomu vrátit

- Večer v den, kdy přišla menstruace.
- Před spaním, když nemůžete usnout kvůli přemýšlení.
- Ráno před odběry nebo kontrolou, jestli máte deset minut v autě.

Opakování je to, co dělá z dechového cvičení něco, co si tělo zapamatuje.

> Nenahrazuje psychologickou ani lékařskou péči. Pokud smutek nebo úzkost
> trvají většinu dní déle než dva týdny, obraťte se prosím na odborníka.`,
      minutes: 12,
      phases: ['trying_naturally', 'preparing_body'],
      dayRange: [30, 500],
      topics: ['psychika', 'sebepece', 'spanek'],
      level: 'comfort',
      hero: 'dusk',
      author: 'Gabi',
      publishedOn: '2026-03-12',
      readAloud: true,
      mediaNote:
        'Psané s tím, že možná pláčete. Jsou tu záměrně dlouhé pauzy a nikdo po vás nic nechce. Nemusíte to projít celé.',
    },
    {
      id: 'plan-rok-snazeni',
      kind: 'article',
      title: 'Rok snažení: co se za tu dobu naučíte o sobě a o vztahu',
      excerpt:
        'Jak vypadá dvanáct cyklů zevnitř. A co z toho stojí za to si nechat.',
      body: `Tenhle text je o roce přirozeného snažení. Ne o léčbě, ne o klinikách.
O té fázi, kterou spousta žen prožije potichu a sama.

## První tři měsíce

Počáteční nadšení. Vysazení antikoncepce, nakoupené vitamíny, poprvé stažená
aplikace. V prvních cyklech ještě funguje logika „to bude ono“ a menstruace
je zklamání, ale ne rána.

Jedno uklidnění, které stojí za zapamatování: **tři neúspěšné cykly nejsou
signál problému.** Většina párů otěhotní během prvního roku a skoro nikdo
z nich netrefil hned první měsíc.

## Kdy se to zlomí

Obvykle mezi čtvrtým a šestým měsícem. Objeví se první ovulační testy, první
čtení diskusí ve tři ráno, první srovnávání s kamarádkami.

A mění se sex. Bez varování se z něj stane úkol s termínem. Tohle popisuje
většina párů a skoro nikdo to nečeká.

## Co dělá tlak s párem

Tahle část je důvod, proč text vznikl.

Muži často zažívají tlak na výkon a nemluví o něm. Ženy si to vykládají jako
nezájem. Vzniká z toho spirála, ve které se dva lidé míjejí ve stejné
místnosti.

Druhá věc je dělba rolí. V drtivé většině párů se zvrhne v to, že všechno
organizuje ona: termíny, testy, vitamíny, čtení, plánování. On „pomáhá“.
To slovo je samo o sobě problém.

Tři věci, které pomáhají:

1. **Nesdělovat mu den cyklu.** Ne kvůli tajení, ale proto, aby konkrétní
   večer nebyl označkovaný.
2. **Zavést blízkost, která není o početí.** Cokoliv, co není sex a co dělá
   z dvou spolupracovníků zase pár.
3. **Domluvit se předem, co kdo potřebuje v den, kdy přijde menstruace.**
   Ona možná potřebuje být sama. On možná potřebuje vědět, že za to nemůže.
   Domluvit se na tom předem je snazší než v tu chvíli.

## Kdy přestat čekat

Odborné společnosti doporučují vyšetření po **roce snahy do 35 let věku**
a po **půl roce od 35 let**.

Spousta párů tuhle hranici o měsíce přetáhne. Nejčastěji ze strachu, že
návštěva lékaře udělá z problému skutečnost. Přitom vyšetření nic nezpůsobí:
jen popíše, co už tam je, a spousta věcí se dá řešit rychle.

## Co si z toho roku nechat

Ať to dopadne jakkoli, zůstane vám znalost vlastního cyklu, schopnost mluvit
s partnerem o nepříjemných věcech a jasno v tom, koho ve svém životě chcete
a koho ne.

> Článek má informativní charakter a nenahrazuje lékařskou péči. Popsané
> zkušenosti jsou individuální a nelze z nich vyvozovat závěry o vaší
> situaci.`,
      minutes: 12,
      phases: ['trying_naturally'],
      dayRange: [60, 420],
      topics: ['psychika', 'vztah', 'partner', 'komunita'],
      level: 'comfort',
      hero: 'taupe',
      author: 'Tým BlooMia',
      publishedOn: '2026-03-26',
    },
    {
      id: 'plan-pribeh-ctrnact-mesicu',
      kind: 'story',
      title: 'Čtrnáct měsíců a jedna věta, která všechno změnila',
      excerpt:
        'Příběh ženy, která rok odkládala návštěvu lékaře, protože se bála, že tím z obavy udělá diagnózu.',
      body: `## Jak to začalo

Vysadila jsem antikoncepci v březnu, přesně podle plánu. Měla jsem třicet dva let, práci, kterou jsem měla ráda, a partnera, se kterým jsme spolu byli šest let. V hlavě jsem měla jasnou představu: do Vánoc to bude, a do léta budu na mateřské.

První tři měsíce jsem prožila v příjemném vzrušení. Kupovala jsem si vitamíny, četla si o folátu a při každé menstruaci jsem si říkala, že je to vlastně ještě brzy.

## Čtvrtý až osmý měsíc

Pak se něco změnilo. Nevzpomenu si na konkrétní den, ale někdy kolem pátého měsíce jsem si poprvé v noci našla na internetu diskusi o tom, jak dlouho to lidem trvalo. Četla jsem do dvou do rána a ráno jsem si koupila balení ovulačních testů.

Od té chvíle jsem věděla o svém cyklu všechno. Věděla jsem, který je den, věděla jsem, jak vypadá hlen, věděla jsem, kdy mám testovat. A věděla jsem taky, kdy mám říct manželovi, že dnes musíme.

Tohle bylo období, na které nejsem hrdá. Nemluvila jsem o ničem jiném. Když mi kamarádka oznámila, že je těhotná, gratulovala jsem jí a pak jsem půl hodiny brečela v autě na parkovišti. Manželovi jsem to neřekla, protože jsem se styděla.

## Devátý měsíc

V devátém měsíci mi řekl větu, kterou jsem neslyšela ráda: „Připadám si jako dárce spermatu."

Byla jsem uražená tři dny. Čtvrtý den mi došlo, že má pravdu. Za devět měsíců jsem se ho ani jednou nezeptala, jak je mu z toho. Bral jsem to tak, že on nemá důvod být smutný, protože jeho tělo netrpí.

Šli jsme na dva víkendy pryč, kde se o dítěti nesmělo mluvit. Nezabralo to v tom smyslu, že bych otěhotněla. Zabralo to v tom, že jsme si zase začali povídat.

## Dvanáctý měsíc, kdy jsem měla jít k lékaři

Rok uplynul v březnu. Věděla jsem, že po roce se má jít k lékaři. Nešla jsem.

Důvod, který jsem si tehdy neuměla přiznat: dokud jsem tam nešla, byla to smůla. Ve chvíli, kdy bych tam šla, mohla by z toho být diagnóza. A já jsem měla panický strach, že mi někdo řekne něco, co se nedá vrátit.

Dva měsíce jsem to odkládala. Objednala jsem se až ve čtrnáctém měsíci, a to jenom proto, že mi máma řekla jednu větu: **„Ty se nebojíš toho, co ti řeknou. Bojíš se toho, co už dávno víš."**

## Co se stalo pak

Nebudu tvrdit, že to dopadlo pohádkově, protože každý příběh je jiný. Řeknu jen tohle: na první konzultaci jsme šli oba, manžel šel na spermiogram a já na odběry a ultrazvuk. Během šesti týdnů jsme věděli, s čím pracujeme.

Ta úleva byla obrovská. Ne proto, že by výsledky byly skvělé. Proto, že jsem konečně přestala hádat.

## Co bych řekla sobě před dvěma lety

- **Ta hranice rok (nebo půl roku, když je vám nad pětatřicet) tam není pro strašení.** Je tam proto, aby se nepromarnil čas.
- **Vyšetření nic nezhorší.** Jen popíše, co už je.
- **Zeptej se ho, jak je jemu.** Dřív než v devátém měsíci.
- **Ta noční čtení diskusí ti nedají nic.** Vůbec nic.
- **A hlavně:** to, že to netrvá tři měsíce, není tvoje vina.

> Osobní zkušenost. Neplyne z ní žádné doporučení pro vaši situaci. O té se poraďte se svým lékařem.`,
      minutes: 8,
      phases: ['trying_naturally'],
      dayRange: [120, 500],
      topics: ['psychika', 'komunita', 'vztah', 'klinika'],
      level: 'comfort',
      hero: 'blush',
      author: 'Tým BlooMia',
      publishedOn: '2026-04-09',
    },
    {
      id: 'plan-pribeh-rozhodnuti-ve-38',
      kind: 'story',
      title: 'Rozhodla jsem se v osmatřiceti a nikdo mi netleskal',
      excerpt:
        'Příběh o tom, jaké je začít plánovat dítě ve chvíli, kdy vám všichni říkají, že už je pozdě.',
      body: `## Věta, kterou jsem slyšela nejčastěji

„No, tak to jsi to nechala hodně na poslední chvíli."

Slyšela jsem ji od kolegyně, od kamarádky z gymnázia i od ženy v čekárně. Nikdo z nich to nemyslel zle, což je na tom to nejhorší. Byla to jen taková samozřejmá poznámka, kterou si lidi dovolí, protože o věku ženy má názor každý.

Bylo mi osmatřicet, měla jsem za sebou rozvod a rok a půl vztahu s někým, s kým jsem si poprvé dokázala představit rodinu. Nenechala jsem to na poslední chvíli. Jen jsem do třiceti pěti nebyla s tím správným člověkem a to není něco, co si člověk naplánuje.

## Co jsem udělala první

Věděla jsem, že mám míň času než třicetiletá kamarádka, a rozhodla jsem se s tím pracovat, ne se tím trápit.

Objednala jsem se ke gynekoložce hned. Ne po půl roce, hned. Řekla jsem jí: je mi osmatřicet, chci dítě, co mám vědět a co mám udělat. Byla to nejlepší schůzka celého toho roku, protože jsem poprvé dostala konkrétní plán místo obecného povzbuzování.

Zjistili jsme si výchozí stav. Ultrazvuk, hormony, štítná žláza. Partner šel na spermiogram, aniž bych ho musela přemlouvat. Přišlo mu logické, že když jdu já, jde i on.

## Co bylo těžké

**Tempo.** Když víte, že hodiny běží, každý měsíc je hlasitější. Nedokázala jsem být v klidu a už mě nebavilo předstírat, že jsem.

**Ticho okolí.** Kamarádky z mého ročníku měly děti na druhém stupni. Neměla jsem s kým sdílet, protože moje fáze nikoho z nich nezajímala. Trvalo mi několik měsíců, než jsem našla lidi ve stejné situaci, a bylo to to nejlepší, co jsem udělala.

**Vlastní hlava.** Nejvíc mi ubližovalo, když jsem si sama počítala, kolik mi bude, až půjde do školy. Ta věta neměla žádný praktický přínos. Jen bolela.

## Co pomohlo

- **Konkrétní plán s termíny.** Věděla jsem, co se stane, když to do daného data nevyjde. Ten následující krok byl domluvený předem a to mě drželo nad vodou.
- **Nečekat.** Přeskočila jsem fázi „ještě to půl roku zkusíme sami" a bylo to správně.
- **Přestat se ptát lidí, kteří to nemají zažité.** Rada od někoho, kdo otěhotněl v šestadvaceti napoprvé, je k ničemu, i když je myšlená hezky.
- **Jedna věta pro obranu.** Naučila jsem se říkat: „Děkuju, řešíme to s lékařem." Tečka. Funguje na kohokoli.

## Co bych vzkázala ženě, které je nad pětatřicet a váhá

Nebudu vám tvrdit, že věk nehraje roli. Hraje a bylo by nefér tvrdit opak. Ale rozdíl mezi ženou, která začne řešit plodnost v pětatřiceti, a tou, která to odloží na osmatřicet, je větší než rozdíl mezi třiceti a pětatřiceti.

Takže: **jestli váháte, jestli je pozdě, nejlepší reakcí na to váhání je objednat se k lékaři.** Ne proto, že by vám něco bylo. Proto, abyste přestala hádat a začala vědět.

A ještě jedna věc. Nikdo mi netleskal a bylo mi to nakonec jedno. To rozhodnutí bylo moje, a i kdyby dopadlo jakkoli, byla to nejsprávnější věc, kterou jsem v tom roce udělala.

> Osobní zkušenost jedné ženy. Nejde o lékařské doporučení. Vaši situaci posoudí váš lékař.`,
      minutes: 7,
      phases: ['thinking', 'trying_naturally', 'preparing_body'],
      topics: ['psychika', 'komunita', 'klinika'],
      level: 'comfort',
      hero: 'pearl',
      author: 'Tým BlooMia',
      publishedOn: '2026-04-23',
    },
    {
      id: 'plan-kviz-plodnost-zaklady',
      kind: 'quiz',
      title: 'Kvíz: rozumíte svému cyklu?',
      excerpt: 'Osm otázek, po kterých budete vědět, jestli plodné okno počítáte správně.',
      body: `## K čemu tenhle kvíz je

Není to zkouška a nikdo vám nedá známku. Je to způsob, jak si za pět minut ověřit, jestli nepracujete s některým z mýtů, které se o početí běžně šíří.

U každé otázky najdete vysvětlení. A to je ve skutečnosti ta hodnotná část. Klidně otázky rovnou proklikejte a čtěte jen odpovědi.

**Kvíz má informativní charakter a nenahrazuje lékařskou péči.** Pokud si nejste jistá čímkoli ohledně svého cyklu, patří ta otázka vašemu gynekologovi.`,
      minutes: 6,
      phases: ['trying_naturally', 'preparing_body'],
      dayRange: [0, 300],
      topics: ['hormony', 'vysledky', 'cekani'],
      level: 'essential',
      hero: 'linen',
      author: 'Tým BlooMia',
      reviewedBy: 'Odborně garantováno – reprodukční medicína',
      publishedOn: '2026-05-07',
      boost: 0.25,
      quiz: [
        {
          q: 'Kdy zhruba nastává ovulace?',
          options: [
            'Vždy 14. den cyklu',
            'Zhruba 14 dní před další menstruací',
            'Zhruba 14 dní po skončení menstruace',
            'V polovině cyklu, ať je jakkoli dlouhý',
          ],
          correct: 1,
          explain:
            'Druhá fáze cyklu bývá poměrně stabilní, kolem dvanácti až čtrnácti dní. Proměnlivá je fáze první. Proto se ovulace odhaduje zpětně od očekávané další menstruace. U cyklu délky 33 dní tedy padá spíš kolem 19. dne než na 14. den.',
        },
        {
          q: 'Jak dlouhé je plodné okno?',
          options: [
            'Jen den ovulace',
            'Zhruba šest dní končících ovulací',
            'Zhruba šest dní začínajících ovulací',
            'Celá druhá polovina cyklu',
          ],
          correct: 1,
          explain:
            'Spermie přežívají v příznivém hlenu až pět dní, vajíčko je oplodnitelné zhruba dvanáct až dvacet čtyři hodin. Plodné okno tedy tvoří pět dní před ovulací plus den ovulace. A končí, ne začíná, ovulací.',
        },
        {
          q: 'Co znamená vzestup bazální teploty?',
          options: [
            'Že ovulace právě probíhá a je čas na styk',
            'Že ovulace nejspíš už proběhla',
            'Že jste těhotná',
            'Že se blíží ovulace za dva dny',
          ],
          correct: 1,
          explain:
            'Teplotu zvyšuje progesteron, který se tvoří až po ovulaci. Teplota tedy ovulaci potvrzuje zpětně a v daném cyklu už na načasování styku nepomůže. K plánování slouží spíš cervikální hlen a LH testy.',
        },
        {
          q: 'Pozitivní ovulační (LH) test znamená, že ovulace nastane přibližně:',
          options: ['Za 24 až 36 hodin', 'Za pět dní', 'Právě teď', 'Už proběhla před dvěma dny'],
          correct: 0,
          explain:
            'Vzestup LH předchází ovulaci zhruba o den až den a půl. Proto se doporučuje mít styk v den pozitivního testu a den následující. Pozitivní test ale neznamená stoprocentní jistotu, že ovulace proběhla. Potvrdí ji až vzestup teploty nebo ultrazvuk.',
        },
        {
          q: 'Jak vypadá hlen v nejplodnějších dnech?',
          options: [
            'Hustý, bílý a lepkavý',
            'Čirý, kluzký a tažný jako syrový vaječný bílek',
            'Žádný. V plodných dnech bývá sucho',
            'Nažloutlý a hustý',
          ],
          correct: 1,
          explain:
            'Vlivem estrogenu hlen řídne a stává se tažným. Takový hlen spermie vyživí a umožní jim projít krčkem. Po ovulaci vlivem progesteronu během jednoho až dvou dnů zhoustne nebo zmizí. Hlen se zápachem, zeleným či sýrovitým vzhledem není plodný hlen, ale důvod ke kontrole u lékaře.',
        },
        {
          q: 'Po jaké době neúspěšného snažení se doporučuje vyhledat lékaře, je-li vám 36 let?',
          options: ['Po dvou letech', 'Po roce', 'Po šesti měsících', 'Až po třech letech'],
          correct: 2,
          explain:
            'Odborná doporučení uvádějí hranici dvanácti měsíců do 35 let a šesti měsíců od 35 let výš. Od čtyřiceti se doporučuje neotálet vůbec. A pokud máte nepravidelný cyklus, silné bolestivé menstruace, známou endometriózu, PCOS nebo problém na straně partnera, patříte k lékaři hned bez ohledu na dobu snažení.',
        },
        {
          q: 'Která z těchto věcí prokazatelně ovlivňuje šanci na otěhotnění?',
          options: [
            'Poloha při pohlavním styku',
            'Ležení s nohama nahoře po styku',
            'Používání běžného lubrikantu',
            'Fáze měsíce',
          ],
          correct: 2,
          explain:
            'Běžné lubrikanty mohou zhoršit pohyblivost spermií, pokud lubrikant potřebujete, vyberte v lékárně variantu označenou jako vhodná při snaze o početí. Poloha, ležení po styku ani fáze měsíce na výsledek vliv nemají.',
        },
        {
          q: 'Jak dlouho trvá tvorba spermií?',
          options: ['Zhruba tři dny', 'Zhruba tři týdny', 'Zhruba tři měsíce', 'Zhruba rok'],
          correct: 2,
          explain:
            'Spermatogeneze trvá přibližně sedmdesát až devadesát dní. Proto se změny životosprávy u muže (konec kouření, omezení tepla, úprava hmotnosti) projeví na spermiogramu zhruba za tři měsíce. Stejný časový horizont platí i pro dozrávání vajíčka, a právě proto má příprava těla smysl začít s čtvrtročním předstihem.',
        },
      ],
    },
  ],
  dailyCards: [
    {
      id: 'plan-dc-priprava-den-1',
      phases: ['preparing_body'],
      day: 1,
      headline: 'Dnes začíná devadesát dní, které mají smysl',
      body: 'Vajíčko, které se uvolní zhruba za tři měsíce, právě teď prochází zráním. Spermie, které se za tři měsíce budou podílet na oplození, se právě začínají tvořit. To, co změníte od dneška, se opravdu stihne projevit.',
      whatsHappening: [
        'Ve vaječníku probíhá zrání folikulů, které bude trvat několik měsíců',
        'Ještě nemusíte nic měřit, počítat ani sledovat',
        'Nejcennější je teď rozhodnutí, ne dokonalé provedení',
      ],
      task: 'Zapište si dnešní datum jako den 1. Pak si do kalendáře dejte jednu jedinou položku: objednat se ke gynekologovi na předkoncepční prohlídku.',
      reflection: 'Co v tomhle období nechci obětovat, i kdyby to trvalo dlouho?',
      tip: 'Nezačínejte vším najednou. Ženy, které v prvním týdnu změní jídlo, spánek, pohyb i návyky, obvykle do měsíce skončí. Vyberte si jednu věc.',
    },
    {
      id: 'plan-dc-priprava-folat',
      phases: ['preparing_body'],
      dayRange: [0, 7],
      headline: 'Folát není doplněk navíc, je to základ',
      body: 'Uzávěr neurální trubice plodu proběhne velmi brzy. Často dřív, než vůbec zjistíte, že jste těhotná. Právě proto se doplňování folátu doporučuje zahájit už teď, ne až po pozitivním testu. Konkrétní přípravek a dávku vám určí lékař podle vaší anamnézy.',
      whatsHappening: [
        'V lékárně na vás čeká desítka podobných krabiček. Liší se hlavně cenou a formou folátu',
        'Většina prenatálních přípravků obsahuje i jód a vitamin D',
        'Vitamin D má smysl doplňovat podle naměřené hladiny, ne odhadem',
      ],
      task: 'Vyfoťte složení všeho, co teď užíváte, včetně bylinek a čajů. Fotku vezměte na první kontrolu k lékaři.',
      reflection: 'Beru doplňky proto, že mi je někdo doporučil, nebo proto, že vím, k čemu jsou?',
      tip: 'Nastavte si připomínku na stejnou denní dobu. U doplňků je pravidelnost důležitější než značka.',
    },
    {
      id: 'plan-dc-priprava-navyky',
      phases: ['preparing_body'],
      dayRange: [8, 20],
      headline: 'Alkohol pryč, káva nejspíš zůstat může',
      body: 'Při snaze o dítě je nejbezpečnější množství alkoholu žádné, hlavně proto, že o prvních týdnech těhotenství se obvykle nedozvíte hned. U kofeinu se odborná doporučení shodují spíš na omezení než na úplném vysazení; přesnou míru proberte s lékařem.',
      whatsHappening: [
        'První dva týdny bez alkoholu bývají společensky nepříjemnější než fyzicky',
        'Kofein je i v černém a zeleném čaji, kole, hořké čokoládě a energetických nápojích',
        'Pokud kouříte, tohle je nejlepší možný okamžik stanovit konkrétní datum konce',
      ],
      task: 'Vymyslete si jednu větu do společnosti pro dnešek. „Dnes nepiju, mám ráno práci." Nikdo se dál neptá.',
      reflection: 'Které z těch tří věcí se vzdávám nejhůř a proč?',
      tip: 'Kupte si doma jeden nealkoholický nápoj, který máte skutečně ráda. Odříkání se snáší líp, když má náhradu.',
    },
    {
      id: 'plan-dc-priprava-spanek',
      phases: ['preparing_body'],
      dayRange: [21, 35],
      headline: 'Spánek je hormonální nástroj, ne odměna',
      body: 'Osa, která řídí váš cyklus, je citlivá na světlo, tmu a stresové hormony. Nepravidelný režim ji rozhodí spolehlivěji než většina věcí, kterými se v přípravě trápíte. Nejúčinnější změna je přitom jednoduchá: stejný čas vstávání sedm dní v týdnu.',
      whatsHappening: [
        'Usínání pod kontrolou nemáte, čas vstávání ano',
        'Deset minut na denním světle dopoledne ukotví rytmus lépe než jakýkoli doplněk',
        'Přípravky na spaní ani melatonin neužívejte v tomhle období bez konzultace s lékařem',
      ],
      task: 'Nastavte si dnes večerní alarm, ne ranní. Připomínku, že za hodinu jdete spát.',
      reflection: 'Co konkrétně mě večer drží vzhůru? Únava, nebo hlava?',
      tip: 'Dejte nabíječku telefonu mimo ložnici. Je to banální rada, která funguje víc než všechny ostatní dohromady.',
      callDoctorIf: [
        'Chrápete a partner popisuje, že se vám v noci zastavuje dech',
        'Nespavost trvá déle než měsíc a ovlivňuje váš běžný den',
        'Budíte se v noci zpocená a s bušením srdce',
      ],
    },
    {
      id: 'plan-dc-priprava-partner',
      phases: ['preparing_body'],
      dayRange: [36, 55],
      headline: 'Jeho devadesát dní běží taky',
      body: 'Faktor na straně muže se podílí zhruba v polovině případů neplodnosti a tvorba spermií trvá přibližně tři měsíce. To, co partner změní teď, se projeví přesně v době, kdy to budete potřebovat. A většina jeho položek je jednodušší než ta vaše.',
      whatsHappening: [
        'Teplo škodí. Sauna, horké koupele, vyhřívaná sedačka, notebook na klíně',
        'Kouření a alkohol zhoršují počet, pohyblivost i tvar spermií',
        'Užívání testosteronu nebo anabolik v minulosti je zásadní informace pro lékaře',
      ],
      task: 'Pošlete partnerovi checklist pro partnera. Nevysvětlujte ho, jen pošlete. Vysvětlování z toho dělá výtku.',
      reflection: 'Ptala jsem se ho někdy, jak je z toho jemu?',
      tip: 'Spermiogram je levné, rychlé a neinvazivní vyšetření. Nemá smysl s ním čekat rok. Dá se udělat kdykoli.',
    },
    {
      id: 'plan-dc-priprava-pohyb',
      phases: ['preparing_body'],
      dayRange: [56, 75],
      headline: 'Hýbat se ano, trestat se ne',
      body: 'Přiměřený pravidelný pohyb zlepšuje citlivost na inzulin, spánek i schopnost zvládat stres. A to všechno hraje ve váš prospěch. Škodí až kombinace extrémní zátěže s nízkým příjmem energie, na kterou cyklus reaguje jako první.',
      whatsHappening: [
        'Rychlá chůze se počítá jako pohyb, opravdu',
        'Silový trénink dvakrát týdně pomůže metabolismu i tomu, jak se budete cítit v těhotenství',
        'Výrazná podváha i nadváha mohou narušovat ovulaci. Obojí je řešitelné postupně',
      ],
      task: 'Naplánujte si do kalendáře tři konkrétní procházky na příští týden. S časem, ne jen s předsevzetím.',
      reflection: 'Cítím se po svém tréninku druhý den líp, nebo hůř?',
      tip: 'Drastické diety a rychlé hubnutí jsou pro cyklus stresová situace. Pokud plánujete výraznější redukci, proberte s lékařem její načasování.',
      callDoctorIf: [
        'Menstruace se zkrátila, zeslábla nebo vynechává',
        'Máte trvalou únavu, která nemizí ani po odpočinku',
        'Opakují se u vás zranění nebo časté infekce',
      ],
    },
    {
      id: 'plan-dc-priprava-cyklus',
      phases: ['preparing_body'],
      dayRange: [76, 90],
      headline: 'Poslední kus přípravy: poznejte svůj cyklus',
      body: 'Blíží se konec devadesáti dní a je čas na poslední věc. Vědět, jak vypadá váš cyklus. Nemusíte měřit teplotu ani kupovat testy. Stačí znát první den menstruace, délku cyklu a to, kdy se objevuje kluzký, tažný hlen.',
      whatsHappening: [
        'Ovulace nastává zhruba čtrnáct dní PŘED další menstruací, ne čtrnáct dní po té minulé',
        'Aplikace jsou dobré na zaznamenávání, ale předpověď v nich je jen průměr z minulosti',
        'Za obvyklé rozmezí délky cyklu se považuje zhruba 21 až 35 dní',
      ],
      task: 'Zapište si datum poslední menstruace a nastavte si připomínku, abyste na zápis nezapomněla ani u příští.',
      reflection: 'Vím vůbec, jak dlouhý je můj cyklus? Nebo to jen odhaduju?',
      tip: 'Tři měsíce zápisků jsou pro gynekologa použitelnější informace než hodina vyprávění v ordinaci.',
      callDoctorIf: [
        'Cyklus je kratší než 21 dní nebo delší než 35 dní',
        'Menstruace vynechává nebo se délka cyklů liší o víc než týden',
        'Krvácení je tak silné, že měníte ochranu častěji než po dvou hodinách',
        'Máte bolesti, které vás vyřazují z běžného dne, nebo bolí pohlavní styk',
      ],
    },
    {
      id: 'plan-dc-snazeni-prvni-cyklus',
      phases: ['trying_naturally'],
      dayRange: [0, 14],
      headline: 'První měsíc: nejdůležitější je nezačít počítat',
      body: 'Právě jste vstoupila do fáze, kterou většina párů zvládne během prvního roku. Teď nepotřebujete tabulky ani testy. Potřebujete vědět, kdy je vaše plodné okno, a mít pravidelný sexuální život. Všechno ostatní přijde, pokud bude potřeba.',
      whatsHappening: [
        'Plodné okno trvá zhruba šest dní a končí ovulací',
        'Nejvyšší šanci mají dva až tři dny PŘED ovulací',
        'Pravidelný styk dvakrát až třikrát týdně trefí plodné okno bez počítání',
      ],
      task: 'Rozhodněte se dnes pro jednu ze dvou strategií: buď sledovat plodné okno, nebo mít pravidelný sex bez počítání. Obě jsou správně, jen si vyberte.',
      reflection: 'Chci tuhle fázi prožít jako projekt, nebo jako součást života?',
      tip: 'Šance na otěhotnění v jednom cyklu se u zdravého páru pohybuje řádově kolem dvaceti procent. Neúspěch v prvním měsíci není informace o ničem.',
    },
    {
      id: 'plan-dc-snazeni-prvni-cekani',
      phases: ['trying_naturally'],
      dayRange: [15, 29],
      headline: 'Čekání, které se bude opakovat',
      body: 'Druhá polovina cyklu je poprvé, kdy zjistíte, jak dlouho umí trvat dva týdny. Příznaky, které teď hledáte, jsou bohužel v téhle fázi stejné u těhotenství i u blížící se menstruace. Obojí způsobuje progesteron.',
      whatsHappening: [
        'Napětí v prsou, únava a nafouklé břicho patří k druhé fázi cyklu, ať už těhotná jste, nebo ne',
        'Test dřív než v očekávaný den menstruace často jen prodlouží nejistotu',
        'Každé předčasné testování znamená o jedno zklamání v cyklu navíc',
      ],
      task: 'Stanovte si jedno konkrétní datum, kdy budete testovat. Napište si ho a víc o tom nepřemýšlejte.',
      reflection: 'Co bych dnes dělala, kdybych na to celé zrovna nemyslela? Dá se aspoň část toho udělat?',
      tip: 'Nevyhledávejte příznaky na internetu. Najdete tam obojí. A nakonec uvěříte tomu, co chcete slyšet.',
    },
    {
      id: 'plan-dc-snazeni-druhy-mesic',
      phases: ['trying_naturally'],
      dayRange: [30, 59],
      headline: 'Druhý měsíc: ovulaci nepočítejte od minulé menstruace',
      body: 'Nejčastější chyba v celém snažení: pravidlo „ovulace je čtrnáctý den" platí jen u osmadvacetidenního cyklu. Ovulace nastává zhruba čtrnáct dní PŘED další menstruací. U delšího cyklu tedy výrazně později, než čekáte.',
      whatsHappening: [
        'U cyklu délky 32 dní padá ovulace spíš kolem 18. dne',
        'Cervikální hlen předpovídá plodné dny dopředu, bazální teplota je potvrdí až zpětně',
        'LH test zachytí hormonální signál zhruba den až den a půl před ovulací',
      ],
      task: 'Spočítejte si podle délky svých posledních cyklů, kdy u vás ovulace nejspíš nastává. Zabere to tři minuty a změní to načasování.',
      reflection: 'Kolik energie mi tohle sledování bere a kolik mi dává?',
      tip: 'Když se objeví čirý, kluzký a tažný hlen podobný syrovému vaječnému bílku, plodné okno je otevřené. Je to nejlevnější a nejspolehlivější signál, který máte.',
    },
    {
      id: 'plan-dc-snazeni-treti-mesic',
      phases: ['trying_naturally'],
      dayRange: [60, 89],
      headline: 'Třetí měsíc a pořád nic není zpráva',
      body: 'Tři neúspěšné cykly nejsou signál problému. Většina párů otěhotní během prvního roku a téměř nikdo netrefí první měsíc. To, co teď cítíte, je normální. A zároveň to neznamená, že se máte tvářit v pohodě.',
      whatsHappening: [
        'Kolem třetího měsíce většina žen poprvé začne číst diskuse ve dvě ráno',
        'Zklamání z menstruace se v tomhle období často promění v truchlení nad konkrétní představou',
        'Je běžné se stydět za to, že vás cizí těhotenství zabolí',
      ],
      task: 'Napište si na papír tři věty: co v tomhle měsíci zvládám dobře, co je moje a co je biologie, koho poprosím o pomoc, až bude nejhůř.',
      reflection: 'Komu jediné bych o tom chtěla říct, aniž by mi radil?',
      tip: 'Když vám někdo řekne „hlavně se uvolni", není to rada. Je to jeho bezradnost. Nemusíte na to reagovat.',
    },
    {
      id: 'plan-dc-snazeni-ctvrty-mesic',
      phases: ['trying_naturally'],
      dayRange: [90, 119],
      headline: 'Čtvrtý měsíc: hlídejte, ať se sex nestane úkolem',
      body: 'Právě v tomhle období se u většiny párů z blízkosti stane provoz. Nejde o vaši chybu ani o jeho nezájem. Je to logický důsledek toho, že jste do postele pustili kalendář. Dá se to vrátit, ale je potřeba to udělat vědomě.',
      whatsHappening: [
        'Věta „dnes musíme" je jeden z nejspolehlivějších způsobů, jak způsobit potíže s erekcí u zdravého muže',
        'Muži v téhle fázi často zažívají tlak na výkon a nemluví o něm',
        'Sex mimo plodné okno bývá první věc, která zmizí',
      ],
      task: 'Domluvte se dnes na jedné věci: jednou týdně blízkost, u které je předem jasné, že není o početí.',
      reflection: 'Kdy naposledy jsme spolu byli, aniž bych přitom myslela na cyklus?',
      tip: 'Zkuste mu neříkat, který je den. Většina mužů funguje mnohem líp bez datového vstupu.',
    },
    {
      id: 'plan-dc-snazeni-pul-roku',
      phases: ['trying_naturally'],
      dayRange: [120, 179],
      headline: 'Blíží se půl roku. Kolik vám je?',
      body: 'Tahle otázka teď rozhoduje o tom, co dělat dál. Od 35 let výš se doporučuje vyhledat lékaře po šesti měsících snažení, do 35 let po dvanácti. Není to trest. Je to hranice, která existuje proto, aby se nepromarnil čas.',
      whatsHappening: [
        'Do 35 let: hranice je dvanáct měsíců, máte tedy ještě půl roku',
        'Od 35 let: půl roku je vaše hranice, objednejte se',
        'Od 40 let: neotálejte vůbec',
      ],
      task: 'Podívejte se do kalendáře, kdy jste začali. Napište si konkrétní datum, kdy se objednáte, pokud to do té doby nevyjde.',
      reflection: 'Odkládám návštěvu lékaře proto, že není potřeba, nebo proto, že se bojím odpovědi?',
      tip: 'Objednejte se ke gynekologovi a jděte tam oba. Diagnostika páru bez spermiogramu je poloviční a zbytečně dlouhá.',
      callDoctorIf: [
        'Cyklus je nepravidelný, kratší než 21 dní nebo delší než 35 dní',
        'Máte silné bolesti při menstruaci, které vás vyřazují z běžného dne',
        'Bolí vás pohlavní styk',
        'V anamnéze máte operaci v malé pánvi, zánět vejcovodů nebo mimoděložní těhotenství',
      ],
    },
    {
      id: 'plan-dc-snazeni-sedmy-mesic',
      phases: ['trying_naturally'],
      dayRange: [180, 209],
      headline: 'Půl roku za vámi',
      body: 'Pokud je vám 35 let nebo víc, tohle je váš moment. Doporučená hranice je za vámi a je čas objednat se k lékaři. Pokud je vám méně, máte ještě půl roku, ale i tak dává smysl začít si připravovat podklady.',
      whatsHappening: [
        'Základní diagnostika neplodnosti bývá součástí hrazené péče',
        'Část odběrů je vázaná na konkrétní den cyklu, proto se výsledky skládají několik týdnů',
        'Spermiogram se dá objednat kdykoli a nezávisle na vašem cyklu',
      ],
      task: 'Sesbírejte zápisky o cyklu za poslední tři měsíce do jedné složky. Až budete v ordinaci, budete ráda.',
      reflection: 'Co bych potřebovala slyšet, abych se cítila líp. Ujištění, nebo konkrétní plán?',
      tip: 'Na první konzultaci si dělejte poznámky. Za dva dny si polovinu toho, co zaznělo, nebudete pamatovat.',
    },
    {
      id: 'plan-dc-snazeni-osmy-devaty',
      phases: ['trying_naturally'],
      dayRange: [210, 269],
      headline: 'Tohle je nejtišší část celé cesty',
      body: 'Osmý a devátý měsíc bývá období, kdy už nadšení dávno vyprchalo a zároveň ještě nemáte žádné odpovědi. Je to fáze, ve které se spousta žen cítí nejvíc sama, hlavně proto, že o ní nikomu neřekly.',
      whatsHappening: [
        'Únava z opakování je reálná a nemá nic společného s vaší silou',
        'Každý cyklus končí koncem konkrétní představy, ne jen negativním testem',
        'Je běžné, že vás začnou obtěžovat i dobře myšlené otázky od rodiny',
      ],
      task: 'Připravte si jednu obrannou větu na dotazy okolí. Například: „Řešíme to a nechci o tom teď mluvit, děkuju." Naučte se ji nazpaměť.',
      reflection: 'Mám v životě aspoň jednu věc, která má vlastní kalendář a vlastní úspěchy, nezávislé na tomhle?',
      tip: 'Psychologická podpora v tomhle období není nadstandard. Řada center asistované reprodukce má vlastního psychologa a poradenství se dá najít i mimo ně.',
      callDoctorIf: [
        'Smutek nebo úzkost trvají většinu dní déle než dva týdny',
        'Nemůžete spát nebo naopak nemůžete vstát',
        'Objevují se panické stavy',
        'Máte myšlenky, že by bylo lepší tu nebýt. V takovém případě vyhledejte pomoc okamžitě',
      ],
    },
    {
      id: 'plan-dc-snazeni-desaty-jedenacty',
      phases: ['trying_naturally'],
      dayRange: [270, 329],
      headline: 'Připravte si půdu, ať za dva měsíce neztrácíte čas',
      body: 'Roční hranice se blíží. Věci, které můžete udělat už teď, vám pak ušetří týdny čekání. Starší nálezy, seznam léků, zápisky o cyklu a hlavně partnerův spermiogram, který se dá objednat kdykoli.',
      whatsHappening: [
        'Spermiogram je levné a neinvazivní vyšetření, které nemá smysl odkládat',
        'Výsledek mimo referenční pásmo není verdikt. Parametry kolísají a vyšetření se obvykle opakuje',
        'Horečnaté onemocnění může výsledek zhoršit na několik týdnů, proto se po nemoci odkládá',
      ],
      task: 'Zavolejte dnes na jedno místo a zjistěte, jak se u vás objednává spermiogram a jak dlouhá je čekací doba.',
      reflection: 'Co konkrétně mi brání jít k lékaři dřív, než musím?',
      tip: 'Až budete výsledek probírat s partnerem, mluvte o tom jako o společné diagnostice. Nikdy ho nepoužívejte jako argument v hádce.',
    },
    {
      id: 'plan-dc-snazeni-rok',
      phases: ['trying_naturally'],
      dayRange: [330, 365],
      headline: 'Rok. Teď už opravdu k lékaři',
      body: 'Dvanáct měsíců pravidelného nechráněného styku bez otěhotnění je hranice, po které odborná doporučení jednoznačně říkají: je čas na vyšetření. Neznamená to, že je něco špatně. Znamená to, že přestanete hádat.',
      whatsHappening: [
        'Vyšetření nic nezhorší, jen popíše, co už je',
        'Do diagnostiky patří oba partneři, hned na začátku',
        'Kompletní obrázek se obvykle skládá několik týdnů kvůli vazbě odběrů na cyklus',
      ],
      task: 'Objednejte se ještě dnes. Ne „tenhle týden". Dnes. Jeden telefonát, pět minut.',
      reflection: 'Čeho se na tom vyšetření bojím nejvíc? A je ten strach větší než další rok nejistoty?',
      tip: 'Sepište si otázky do telefonu předem. V ordinaci na ně spolehlivě zapomenete.',
      callDoctorIf: [
        'Silné krvácení mimo menstruaci',
        'Horečka spolu s bolestí v podbřišku',
        'Náhlá prudká bolest v podbřišku, zvlášť jednostranná',
        'Pozitivní těhotenský test spolu s bolestí a krvácením. Může jít o mimoděložní těhotenství, to je akutní stav',
      ],
    },
    {
      id: 'plan-dc-snazeni-pres-rok',
      phases: ['trying_naturally'],
      dayRange: [366, 720],
      headline: 'Přes rok. Co teď opravdu pomůže',
      body: 'Jestli jste u lékaře ještě nebyla, tohle je jediná věc, která má dnes cenu. Jestli už tam jste a čekáte na výsledky, je normální, že vás vyčerpává i samotné čekání. A je v pořádku o tom nahlas říct.',
      whatsHappening: [
        'Pauza od snažení pomáhá psychice, ale není léčebnou metodou, pokud si ji dáte, dejte si ji vědomě',
        'Pokud jste splnila časové kritérium a přesto vám bylo vyšetření odmítnuto bez zdůvodnění, máte právo na druhý názor',
        'Sekundární neplodnost je reálná i u žen, které už jednou otěhotněly',
      ],
      task: 'Napište si jednu větu o tom, co potřebujete od partnera v příštím měsíci. Pak mu ji řekněte nahlas.',
      reflection: 'Co z toho, co teď dělám, dělám proto, že pomáhá. A co jen proto, abych měla pocit kontroly?',
      tip: 'Noční čtení diskusí vám nedá nic. Když už hledáte informace, hledejte je ráno a s konkrétní otázkou, se kterou pak půjdete k lékaři.',
      callDoctorIf: [
        'Menstruace úplně vynechává déle než tři měsíce',
        'Objevuje se špinění mezi cykly nebo po styku',
        'Bolesti v podbřišku sílí nebo se objevují mimo menstruaci',
      ],
    },
    {
      id: 'plan-dc-snazeni-pcos',
      phases: ['trying_naturally', 'preparing_body'],
      dayRange: [0, 720],
      modifiers: ['pcos'],
      headline: 'S PCOS platí trochu jiná pravidla sledování',
      body: 'Při syndromu polycystických ovarií bývá bazální hladina LH trvale vyšší, takže ovulační testy mohou vycházet pozitivně opakovaně, aniž by ovulace nastala. Sledování cyklu proto může být zavádějící a postup je vhodné probrat s lékařem.',
      whatsHappening: [
        'Nepravidelný nebo velmi dlouhý cyklus je u PCOS častý a není vaší chybou',
        'I mírná úprava hmotnosti mívá u PCOS na obnovení ovulace znatelný vliv',
        'S PCOS patříte k lékaři bez ohledu na to, jak dlouho se snažíte',
      ],
      task: 'Sepište si do poznámek délku svých posledních tří cyklů. U PCOS je to informace, kterou lékař potřebuje nejdřív.',
      reflection: 'Vím, kdy mám další kontrolu, a co se na ní bude řešit?',
      tip: 'Cervikální hlen bývá u PCOS spolehlivějším vodítkem než LH testy. Konkrétní postup ale vždy proberte se svým gynekologem.',
      callDoctorIf: [
        'Menstruace vynechává déle než tři měsíce',
        'Krvácení je po dlouhé pauze velmi silné nebo dlouhé',
        'Zhoršuje se akné, ochlupení nebo dochází k rychlému nárůstu hmotnosti',
      ],
    },
    {
      id: 'plan-dc-snazeni-muzsky-faktor',
      phases: ['trying_naturally', 'preparing_body'],
      dayRange: [0, 720],
      modifiers: ['male_factor'],
      headline: 'Jeho výsledek není verdikt o něm',
      body: 'Nález mimo referenční pásmo bere spousta mužů jako rozsudek o vlastní mužnosti. Přitom parametry spermiogramu kolísají, ovlivní je nemoc i doba abstinence před odběrem, a odchýlený výsledek se proto obvykle opakuje s odstupem.',
      whatsHappening: [
        'Tvorba spermií trvá zhruba tři měsíce. Efekt změn se hodnotí nejdřív po téhle době',
        'Odstranění tepelné zátěže, konec kouření a úprava hmotnosti mají doložený vliv',
        'Užívání testosteronu nebo anabolik v minulosti musí zaznít u lékaře',
      ],
      task: 'Domluvte se s partnerem na jedné konkrétní změně na příští tři měsíce. Jedné, ne pěti.',
      reflection: 'Mluvíme o tom jako o naší diagnostice, nebo jako o jeho výsledku?',
      tip: 'Neposílejte mu výsledek v esemesce a neinterpretujte ho podle internetu. Nález patří k lékaři, ideálně androloga či urologa se zaměřením na reprodukci.',
    },
    {
      id: 'telo-dc2-1',
      phases: ['preparing_body'],
      dayRange: [0, 13],
      headline: 'Vitamin D se měří, nehádá',
      body: 'Vitamin D patří mezi hodnoty, které se u plánování těhotenství běžně kontrolují z krve. V našich zeměpisných šířkách bývá přes zimu u části lidí nízký, ale odhadovat se to nedá. Nechte si ho změřit a o případném doplňování rozhodne lékař podle výsledku.',
      whatsHappening: [
        'Vitamin D si kůže tvoří ze slunečního záření, od podzimu do jara ho tělo vyrobí málo',
        'Hodnota se zjišťuje z odběru krve, na žádance bývá jako 25-OH vitamin D',
        'Dávka se odvíjí od naměřené hodnoty, ne od toho, co je na krabičce',
        'Volně prodejné neznamená, že si sílu určujete sama',
      ],
      task: 'Zavolejte praktickému lékaři nebo gynekologovi a zeptejte se, jestli vám vitamin D můžou přidat k dalším odběrům.',
      tip: 'Pokud vitamin D už berete, opište si přesný název přípravku a počet jednotek. Lékař se na to zeptá dřív než na cokoliv jiného.',
      callDoctorIf: [
        'Berete vysoké dávky vitaminu D delší dobu bez kontroly hodnoty',
        'Máte onemocnění ledvin, jater nebo štítné žlázy',
        'Máte nevysvětlitelnou svalovou slabost nebo bolesti kostí',
        'Užíváte současně několik přípravků, ve kterých je vitamin D obsažen',
      ],
    },
    {
      id: 'telo-dc2-2',
      phases: ['preparing_body'],
      dayRange: [5, 19],
      headline: 'Štítná žláza mluví do cyklu',
      body: 'Štítná žláza ovlivňuje délku cyklu, ovulaci i průběh případného těhotenství. Proto se u žen, které plánují otěhotnět, kontroluje hodnota TSH a někdy i protilátky. Rozmezí, které se sleduje u plánování, se liší od běžného, a co znamená vaše číslo, řekne lékař.',
      whatsHappening: [
        'TSH je hormon z mozku, který štítnou žlázu řídí, měří se z krve',
        'U plánování se kromě TSH často doplňují protilátky proti štítné žláze',
        'Porucha štítné žlázy může běžet roky bez nápadných příznaků',
        'Pokud už léčbu máte, dávka se v těhotenství obvykle přehodnocuje',
      ],
      task: 'Projděte starší výsledky odběrů a najděte, jestli tam TSH někdy bylo a z jakého data.',
      reflection: 'Co o svojí štítné žláze doopravdy vím a co jenom tuším?',
      callDoctorIf: [
        'V rodině se vyskytuje onemocnění štítné žlázy',
        'Máte dlouhodobě zimomřivost, únavu, vypadávání vlasů nebo zácpu',
        'Máte bušení srdce, třes rukou nebo nevysvětlený úbytek hmotnosti',
        'Váš cyklus je výrazně nepravidelný nebo se v posledním roce změnil',
        'Už léky na štítnou žlázu berete a nebyla vám kontrolována hodnota několik měsíců',
      ],
    },
    {
      id: 'telo-dc2-3',
      phases: ['preparing_body'],
      dayRange: [12, 27],
      headline: 'Vapování se nepočítá jako kompromis',
      body: 'Kouření zhoršuje kvalitu vajíček i spermií a působí i pasivně, tedy i z cigaret partnera. Elektronické cigarety a zahřívaný tabák nejsou bezpečná varianta, jen jiná. Pokud přestáváte, je rozumné vzít si k tomu pomoc, ne to zkoušet čistě vůlí.',
      whatsHappening: [
        'Kouření souvisí s nižší zásobou vajíček a dřívějším nástupem menopauzy',
        'Nikotin z náplní do vapu působí na cévy stejně jako z cigaret',
        'Pasivní kouř v jednom bytě se týká vás obou',
        'Odvykací poradny a linky jsou v Česku dostupné a bývají zdarma',
      ],
      task: 'Zapište si, kolik cigaret nebo potažení denně vám reálně projde, a v jakých třech situacích to nejčastěji je.',
      reflection: 'Co mi kouření nebo vapování v tuhle chvíli nahrazuje?',
      tip: 'Nesnažte se přestat v týdnu, kdy vás čeká odběr, stěhování nebo velký termín v práci. Vyberte si nudný týden, ne krizový.',
    },
    {
      id: 'telo-dc2-4',
      phases: ['preparing_body'],
      dayRange: [20, 35],
      headline: 'Váha jako hormonální údaj, ne estetický',
      body: 'Tuková tkáň se podílí na tvorbě estrogenů, takže výrazně nízká i výrazně vysoká hmotnost může ovlivnit ovulaci. Kliniky proto o BMI mluví, i když to zní nepříjemně. Není to hodnocení vašeho vzhledu ani vaší kázně, je to jedna z proměnných, která se dá někdy posunout.',
      whatsHappening: [
        'Ovulace může vynechávat při velmi nízké i při vysoké hmotnosti',
        'Některé kliniky mají hraniční BMI pro zahájení stimulace, ptejte se na konkrétní číslo té vaší',
        'Prudké hubnutí těsně před cyklem tělu spíš uškodí než pomůže',
        'U PCOS a poruch štítné žlázy má hmotnost jiné pozadí, řeší se spolu s nimi',
      ],
      task: 'Zeptejte se na klinice nebo v ambulanci, jestli mají hraniční BMI a jaké přesně.',
      reflection: 'Dokážu o svojí váze přemýšlet jako o jednom údaji mezi mnoha?',
      tip: 'Pokud se rozhodnete na hmotnosti pracovat, domluvte si to s nutričním terapeutem. Diety z internetu bývají příliš přísné na období, kdy tělo potřebuje stabilitu.',
    },
    {
      id: 'telo-dc2-5',
      phases: ['preparing_body'],
      dayRange: [28, 43],
      headline: 'Ovulaci poznáte i bez proužků',
      body: 'Tělo kolem ovulace mění hlen, teplotu i to, jak se cítíte. Naučit se to číst stojí pár cyklů pozorování a nic to nestojí. Neznamená to, že máte měřit všechno naráz, stačí si vybrat jednu věc a sledovat ji.',
      whatsHappening: [
        'Hlen se před ovulací mění na průhledný, kluzký a tažný, po ovulaci obvykle houstne',
        'Bazální teplota po ovulaci mírně stoupá, ale ukáže to až zpětně',
        'Část žen cítí kolem ovulace tah nebo píchání na jedné straně podbřišku',
        'Někdy se objeví citlivější prsa nebo krátké slabé špinění, u jiných žen nic z toho',
      ],
      task: 'Zaznamenejte si dnes jednu větu o tom, jaký máte hlen. Nic víc, jen dnešní den.',
      reflection: 'Kdy naposledy jsem svému tělu věnovala pozornost bez toho, abych ho hodnotila?',
      tip: 'Bazální teplotu má smysl měřit ihned po probuzení, před vstáváním, pokaždé stejným teploměrem. Jednorázové měření nic neřekne, čte se z křivky.',
    },
    {
      id: 'telo-dc2-6',
      phases: ['preparing_body'],
      dayRange: [36, 51],
      headline: 'Co ovulační test ukazuje a co ne',
      body: 'Ovulační testy zachytí vzestup luteinizačního hormonu, který ovulaci obvykle předchází. Pozitivní test tedy říká, že se ovulace chystá, neříká, že proběhla. U části žen, hlavně u PCOS, mohou testy vycházet pozitivně opakovaně, aniž by to znamenalo ovulaci.',
      whatsHappening: [
        'Test se dělá z moči a reaguje na hormon LH, ne na těhotenský hormon hCG',
        'Vzestup LH bývá krátký, proto se testuje v odpoledních hodinách a někdy dvakrát denně',
        'Slabá čárka se u těchto testů nepočítá jako pozitivní, hodnotí se sytost proti kontrolní čárce',
        'Testy neukazují kvalitu vajíčka ani to, jestli je vejcovod průchodný',
      ],
      task: 'Podívejte se na krabičku svého testu a přečtěte si, jakou citlivost udává a jak má vypadat pozitivní výsledek.',
      tip: 'Testovat od prvního dne cyklu je zbytečné plýtvání. Podle délky vašeho cyklu se dá odhadnout, kdy začít, a klinika vám to ráda upřesní.',
      callDoctorIf: [
        'Testujete několik cyklů po sobě a pozitivní výsledek se nikdy neobjeví',
        'Test vychází pozitivně skoro každý den v cyklu',
        'Váš cyklus je kratší než 21 nebo delší než 35 dní',
        'Cyklus se úplně zastavil na tři měsíce a více',
      ],
    },
    {
      id: 'telo-dc2-7',
      phases: ['preparing_body'],
      dayRange: [44, 59],
      headline: 'Se stresem se dá pracovat prakticky',
      body: 'Stres z léčby nezmizí tím, že si ho zakážete, a nikdo po vás nechce, abyste byla klidná. Co jde ovlivnit, je zátěž kolem: kolik informací denně přijmete, komu o tom říkáte a kolik místa tomu necháte v kalendáři. To jsou konkrétní věci, ne nálada.',
      whatsHappening: [
        'Vyhledávání příběhů na diskuzních fórech často zvyšuje napětí, i když se tváří jako příprava',
        'Opakované vysvětlování situace v práci nebo rodině stojí víc energie, než se čeká',
        'Psychologická podpora je běžnou součástí péče, řada center ji nabízí přímo',
        'Dechová cvičení a pohyb stres nevyléčí, ale pomáhají tělu se ze špičky vrátit dolů',
      ],
      task: 'Vyberte si jednu diskuzní skupinu nebo účet, který vám dlouhodobě ubližuje, a ztlumte ho.',
      reflection: 'Co z toho, co teď nesu, ve skutečnosti nemusím nést sama?',
      tip: 'Nachystejte si jednu větu pro lidi, kteří se ptají: děkuju, teď to neřeším, dám vědět. Nemusíte nikomu nic vysvětlovat.',
    },
    {
      id: 'telo-dc2-8',
      phases: ['preparing_body'],
      dayRange: [52, 67],
      headline: 'Jídlo bez diet a bez viny',
      body: 'Neexistuje jídelníček, který by otěhotnění zařídil, a žádná potravina není vinná, pokud to nevyšlo. Co má oporu, je pravidelnost, dost bílkovin, zelenina a méně alkoholu. Zbytek jsou většinou přehnané sliby.',
      whatsHappening: [
        'Vynechávání celých skupin potravin bez důvodu spíš ubírá živiny, než přidává',
        'Pravidelné jídlo pomáhá stabilnější hladině cukru, což se u PCOS řeší přednostně',
        'Ryby jsou zdrojem omega-3, u dravých druhů se v těhotenství hlídá množství kvůli rtuti',
        'Nepasterizované sýry a syrové maso mají svoje pravidla, která má smysl dodržovat už ve chvíli, kdy se snažíte nebo jdete do transferu. O prvních týdnech těhotenství se ještě neví',
      ],
      task: 'Naplánujte si na zítřek jedno jídlo dopředu, aby vás nezaskočilo. Stačí jedno.',
      reflection: 'Mluvím sama se sebou o jídle způsobem, jakým bych mluvila s kamarádkou?',
      tip: 'Pokud jste teď ve fázi, kdy nezvládáte vařit, je hotové jídlo lepší volba než vynechané jídlo.',
    },
    {
      id: 'telo-dc2-9',
      phases: ['preparing_body'],
      dayRange: [60, 74],
      headline: 'Zubař a další věci vyřídit teď',
      body: 'Některé věci se v těhotenství odkládají, protože se pak dělají hůř. Zubní prohlídka, doléčení chronického zánětu, kontrola očkování nebo doplnění dokumentace patří do doby, kdy máte klid. Vyřízeno teď znamená o jednu starost méně později.',
      whatsHappening: [
        'Zánět dásní se v těhotenství častěji zhoršuje kvůli hormonálním změnám',
        'Rentgen zubů se v těhotenství omezuje, teď je to bez komplikací',
        'U zarděnek a planých neštovic se před otěhotněním zjišťuje, jestli jste imunní',
        'Vyšetření na chlamydie a další infekce bývá součástí předléčebného balíčku',
      ],
      task: 'Objednejte se k zubaři. Když má termín za tři měsíce, tím spíš to udělejte dnes.',
      tip: 'Sepište si na jeden papír, co ještě chcete vyřídit, a ke každé položce telefon. Nemusíte to udělat všechno, ale ať to není v hlavě.',
    },
    {
      id: 'telo-dc2-10',
      phases: ['preparing_body'],
      dayRange: [67, 81],
      headline: 'Zápisky, ze kterých má lékař užitek',
      body: 'Na první konzultaci se ptají na věci, které si z hlavy nikdo nepamatuje. Když si je průběžně zapisujete, ušetříte si dohadování a lékař dostane přesnější obraz. V aplikaci BlooMia si to můžete vést na jednom místě.',
      whatsHappening: [
        'První den cyklu je den plného krvácení, ne špinění, na tenhle údaj se ptají pokaždé',
        'Užitečná je délka cyklu za posledních šest měsíců, ne jen průměr',
        'Seznam všech léků, doplňků a bylinných přípravků včetně dávkování',
        'Data a výsledky předchozích vyšetření, včetně spermiogramu partnera',
      ],
      task: 'Založte si jeden seznam a napište do něj první den svého posledního cyklu a všechny přípravky, které berete.',
      reflection: 'Co bych chtěla, aby lékař o mém posledním roce věděl?',
      tip: 'Vyfoťte si papírové výsledky do telefonu do jednoho alba. Na konzultaci je pak najdete za pár vteřin.',
    },
    {
      id: 'telo-dc2-11',
      phases: ['preparing_body'],
      dayRange: [74, 88],
      headline: 'Doplňky slibují víc, než unesou',
      body: 'Regály jsou plné přípravků na plodnost s odvážnými popisky. Část z nich má rozumné složení, část jen drahé obaly, a některé bylinné směsi mohou zasahovat do hormonů nebo do léčby. Ukažte lékaři všechno, co berete, i to, co si nekupujete v lékárně.',
      whatsHappening: [
        'Kyselina listová (folát) je doplněk s nejjasnější oporou. Bere se už před otěhotněním a konkrétní dávku určuje lékař, u části žen bývá vyšší.',
        'Kombinované přípravky se často překrývají, snadno pak berete stejnou látku dvakrát',
        'Bylinné směsi typu drmek nebo maca nemají u léčby jasné místo a mohou interagovat',
        'Vysoké dávky vitaminu A jsou v období plánování těhotenství nevhodné',
        'Slova jako přírodní a bez chemie neříkají nic o bezpečnosti ani o dávce',
      ],
      task: 'Vysypte na stůl všechno, co berete, a vyfoťte to. Tu fotku vezměte k lékaři.',
      tip: 'Ceny přípravků se liší i několikanásobně při stejném složení. Porovnávejte účinné látky a jejich množství, ne obal.',
      callDoctorIf: [
        'Berete více přípravků naráz a nevíte, jestli se nepřekrývají',
        'Užíváte bylinné směsi na hormonální rovnováhu nebo na cyklus',
        'Po nasazení nového doplňku vám je nevolno nebo máte trávicí potíže',
        'Máte chronické onemocnění nebo berete léky na předpis',
      ],
    },
    {
      id: 'telo-dc2-12',
      phases: ['preparing_body'],
      dayRange: [82, 93],
      headline: 'Kdy přestat čekat a objednat se',
      body: 'Čekání má svoje hranice a přejít je není selhání ani netrpělivost. Obvyklé doporučení je nechat se vyšetřit po roce snažení, a po půl roce, pokud je vám nad pětatřicet. Existují situace, kdy má smysl jít dřív a nečekat vůbec.',
      whatsHappening: [
        'Vyšetření se vždy týká obou, jeho spermiogram bývá první a nejrychlejší krok',
        'Nepravidelný nebo chybějící cyklus je důvod jít bez ohledu na to, jak dlouho zkoušíte',
        'Prodělaný zánět v malé pánvi, operace na vaječnících nebo endometrióza posouvají termín dopředu',
        'Objednací lhůty bývají několik týdnů, čas na objednání se počítá do čekání',
      ],
      task: 'Napište si datum, kdy jste začali zkoušet. Když si ho nepamatujete přesně, napište měsíc.',
      reflection: 'Co mi zatím bránilo objednat se a je to pořád platný důvod?',
      callDoctorIf: [
        'Zkoušíte otěhotnět rok a je vám do 35 let',
        'Zkoušíte otěhotnět půl roku a je vám 35 let a více',
        'Nemáte menstruaci tři měsíce a více nebo je cyklus výrazně nepravidelný',
        'Prodělala jste opakovaný potrat, operaci na vaječnících nebo zánět v malé pánvi',
        'Máte silné bolesti při menstruaci nebo při pohlavním styku',
      ],
    },
  ],
}
